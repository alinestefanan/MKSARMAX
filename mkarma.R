...
EMV.mkarma <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
  ...
  
{
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,]%*%as.matrix(beta) + sum(phi*(ynew[i-ar]  ) ) - sum(theta*error[i-ma])
  ...
  score <- function(z) 
  {
    ...
    score<-c(Ubeta,Uphi,Utheta,Ualpha)#arma
    return(score)
  }#fim score
  ...
  opt <- optim(reg, loglik, score,method = "BFGS", hessian = T, 
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,]%*%as.matrix(beta) + sum(phi*(ynew[i-ar]  ) ) - sum(theta*errorhat[i-ma])
  ...
  for(i in m:1)
  {
    etam[i] <- X[i,1]%*%as.matrix(z$beta0) + sum(phi*(ynew[i+ar])) - sum(theta*errorm[i+ma])
