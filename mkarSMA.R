...
EMV.mkarSMA <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                        tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,]%*%as.matrix(beta) + sum(phi*(ynew[i-ar]) ) - sum(THETA*error[i-MA*S])
  ...
  score <- function(z) 
  {
    ...
    return(c(Ubeta,Uphi,UTHETA,Ualpha))
  }#END score
  ...
  x <- cbind(X[(m+1):n,],P.phi,Q.THETA)#intercept, covariates,ar, MA
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
  ajuste = lm.fit(x, Ynew) 
  #print(ajuste)
  mqo = c(ajuste$coef)
  for(i in 1:length(mqo))#because at ma and MA it will be NA, for set 0 instead
  {
    if (is.na(mqo[i])){
      mqo[i] <- 0#mean(ajuste$residuals)
    }
  }
  library(GenSA)
 ...
 # library(rootSolve)
  ...
  # print(rbind(score(opt$par),gradient(loglik,opt$par)))#tem que dar zero
  # print(rbind(score(opt2$par),gradient(loglik,opt2$par)))#tem que dar zero
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,]%*%as.matrix(beta) + sum(phi*(ynew[i-ar]  ) ) - sum(THETA*errorhat[i-MA*S])
  ...
  for(i in (m+1):n)
  {
    deta.dbeta0[i,]<-  B[(i-m),] +  THETA%*%deta.dbeta0[i-MA*S,]
    deta.dphi[i,]<- A[(i-m),] +   THETA%*%deta.dphi[i-MA*S,]
    deta.dTHETA[i,]<- Rs[(i-m),] +   THETA%*%deta.dTHETA[i-MA*S,]
  ...
  K <- rbind(
    cbind(KBB,KBp,KBQ,KBalpha),
    cbind(KpB,Kpp,KpQ,Kpalpha),
    cbind(KQB,KQp,KQQ,KQalpha),
    cbind(KalphaB,Kalphap,KalphaQ,Kalphaalpha)
  )
  return(K)
  }
  ...
  z$zstat<-z$coeff/sqrt(v)#Z Statistics of Wald's Test
  ...
  {
    first_col<-c("intercept",ar, MA,"alpha estimator")
    result <- matrix(c(first_col,round(c(z$coeff,z$zstat,LI,LS,z$pvalues),4),resp), nrow=length(opt$par), ncol=7, byrow=F)
    colnames(result) <- c("Estimator","EMV","Wald's Statistic","Lower bound","Upper bound","p-value","Wald'S Test result")
    rownames(result)<-c("", rep("ar",length(ar)), rep("MA",length(MA)),"")
  }else{
    first_col<-c("intercept", 1:ncol(exvar),ar, MA,"alpha estimator")
    result <- matrix(c(first_col,round(c(z$coeff,z$zstat,LI,LS,z$pvalues),4),resp), nrow=length(opt$par), ncol=7, byrow=F)
    colnames(result) <- c("Estimator","EMV","Wald's Statistic","Lower bound","Upper bound","p-value","Wald'S Test result")
    rownames(result)<-c("", rep("cov",ncol(exvar)), rep("ar",length(ar)), rep("MA",length(MA)),"")
  }
  ...
  for(i in m:1)
  {
    etam[i] <- X[i,1]%*%as.matrix(z$beta0) + sum(ar*(ynew[i+ar])) - sum(THETA*errorm[i+S*MA])
  ...
  for(i in 1:steps)
  {
    ynew_prev[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) + sum(z$phi*(ynew_prev[n+i-ar]) ) - sum(z$Theta*errorhat[n+i-MA*S])
  ...
  for(i in 1:steps)
  {
    gy_prev[n+i] <- X_prev[n+i,1]*z$beta0  + sum(z$phi*(gy[n+i-ar]) ) - sum(z$Theta*errorhat[n+i-MA*S])
