...
EMV.mkar <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                     tau=0.5,resid=1,aclag=10,steps=12,exvar_steps=matrix(NA, nrow=1, ncol=1, byrow=F),validation=T,graph=T,print=T,check=F
,link="logit")
{
  tau=tau
  k<-0 #default
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,]%*%as.matrix(beta) + sum(phi*(ynew[i-ar] ) )
  ...
  score <- function(z) 
  {
    ...
    B <- matrix(rep(NA,(n-m)*length(beta)),ncol=length(beta))#intercepto e covariáveis
    ...
    Uphi <- matrix(0, ncol=ncol(Ubeta),nrow=nrow(Ubeta))
  ...
  if (opt$conv != 0)
  {
    ...
    z$RMC=1
    z$maic <-100
    z$accuracyforecast<-matrix(c(100,100,100,100,100,0,0), nrow=1, ncol=7, byrow=T)
    return(z)
  }
  ...
  z$MA=MA
  z$delta=m
  z$roots=abs(polyroot(vector.root(z$ar,z$phi)))
  z$RMC=0
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,]%*%as.matrix(beta) + sum(phi*(ynew[i-ar] ) ) 
  ...
  for(i in m:1)
  {
    etam[i] <- X[i,1]%*%as.matrix(z$beta0) + sum(phi*(ynew[i+ar]))
  ...
  if(is.na(exvar_steps[1,1])==F)
  {
    k <- ncol(exvar_steps)
    exvartotal<-matrix(c(exvar,exvar_steps), nrow=ntotal, ncol=k)
    X_prev <- matrix(c(rep(1,ntotal),exvartotal), nrow=ntotal, ncol=(k+1),byrow = F)
  }
  
  for(i in 1:steps)
  {
    ynew_prev[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) + sum(z$phi*(ynew_prev[n+i-ar]) )
  ...
  for(i in 1:steps)
  {
    gy_prev[n+i] <- X_prev[n+i,1]*z$beta0  + sum(z$phi*(gy[n+i-ar]) ) 
