...
EMV.mkarSARX <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                         tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,1]*beta0 + X[i,2:ncol(X)]%*%as.matrix(beta) + sum(ar_par*(ynew[i-ar_ind]-X[i-ar_ind,2:ncol(X)]%*%as.matrix(beta)  ) ) 
  ...
  score <- function(z) 
  {
    ...
    ##############################################################################
    return(c(Ubeta0,Ubeta,Uphi,UPHI,Ualpha))
  }#end score
  ...
  x <- cbind(X[(m+1):n,],P.phi, P.PHI)#intercepto, covariáveis,ar, ma, AR
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,1]*beta0 + X[i,2:ncol(X)]%*%as.matrix(beta) + sum(ar_par*(ynew[i-ar_ind]-X[i-ar_ind,2:ncol(X)]%*%as.matrix(beta)) )
  ...
  obs.inf<-function(y1,muhat)
  {
    ...
    for(i in (m+1):n)
    {
      ...
      for(b in 1:p1)
      {
        for(a in 1:k)
        {
          deta.dbetaphi[a,b,i] <- bA[a,b,(i-m)] 
        }
      }
      for(b in 1:P1)
      {
        for(a in 1:k)
        {
          deta.dbetaPHI[a,b,i]<- BAs[a,b,(i-m)]
        }
      }
      for(b in 1:p1)
      {
    ...
    K <- rbind(
      cbind(KB0B0,KB0B,KB0p,KB0P,KB0alpha),
      cbind(KBB0,KBB,KBp,KBP,KBalpha),
      cbind(KpB0,KpB,Kpp,KpP,Kpalpha),
      cbind(KPB0,KPB,KPp,KPP,KPalpha),
      cbind(KalphaB0,KalphaB,Kalphap,KalphaP,Kalphaalpha)
    )
    return(K)
  }
  K<-obs.inf(y1,muhat)
  ...
  first_col<-c("intercept", 1:k,ar,  AR,"alpha estimator")
  ...
  # for(i in m:1)
  # {
  #   etam[i] <- X[i,1]*z$beta0 +X[i,2:ncol(X)]%*%as.matrix(z$beta)+ sum(ar_par*(ynew[i+ar_ind])-X[i+ar_ind,2:ncol(X)]%*%as.matrix(z$beta))
  ...
  z$deviance <- 2*sum(dt)
  z$dof.dev=(n-m-p1-P1)#desconsidera intercepto do eta e alpha da distribuição
