...
EMV.mkmaSMAX <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                         tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,1]*beta0 + X[i,2:ncol(X)]%*%as.matrix(beta) - sum(ma_par*error[i-ma_ind])
  ...
  score <- function(z) 
  {
    ...
    ##############################################################################
    return(c(Ubeta0,Ubeta,Utheta,UTHETA,Ualpha))
  }#end score
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,1]*beta0 + X[i,2:ncol(X)]%*%as.matrix(beta) - sum(ma_par*errorhat[i-ma_ind])
  ...
  obs.inf<-function(y1,muhat)
  {
    ...
    K <- rbind(
      cbind(KB0B0,KB0B,KB0q,KB0Q,KB0alpha),
      cbind(KBB0,KBB,KBq,KBQ,KBalpha),
      cbind(KqB0,KqB,Kqq,KqQ,Kqalpha),
      cbind(KQB0,KQB,KQq,KQQ,KQalpha),
      cbind(KalphaB0,KalphaB,Kalphaq,KalphaQ,Kalphaalpha)
    )
    return(K)
  }
  K<-obs.inf(y1,muhat)
  ...
  first_col<-c("intercept", 1:k, ma, MA,"alpha estimator")
  ...
  rownames(result)<-c("", rep("cov",k), rep("ma",length(ma)), rep("MA",length(MA)),"")
  ...
  if(steps!=0){
    ...
    for(i in 1:steps)
    {
      ynew_prev[n+i] <- X_prev[n+i,1]*z$beta0 + X_prev[n+i,2:ncol(X_prev)]%*%as.matrix(z$beta) - sum(ma_par*errorhat[n+i-ma_ind])
  ...
  z$deviance <- 2*sum(dt)
  z$dof.dev=(n-m-q1-Q1)#desconsidera intercepto do eta e alpha da distribuição
  ...
  for(i in m:1)
  {
    etam[i] <- X[i,1]*z$beta0 +X[i,2:ncol(X)]%*%as.matrix(z$beta)- sum(ma_par*errorm[i+ma_ind])
