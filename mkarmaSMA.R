...
EMV.mkarmaSMA <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                          tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,]%*%as.matrix(beta) + sum(phi*(ynew[i-ar]#-X[i-ar,]%*%as.matrix(beta)
  ...
  score <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,]%*%as.matrix(beta) + sum(phi*(ynew[i-ar]) ) - sum(ma_par*error[i-ma_ind])
      error[i] <- ynew[i]-eta[i] #residuals on the predictor scale
    }
    ...
    for(i in (m+1):n)
    {
      ...
      deta.dphi[i,]<- A[(i-m),] +  ma_par%*%deta.dphi[i-ma_ind,]
    ...
    return(c(Ubeta,Uphi,Utheta,UTHETA,Ualpha))
  }#fim score
  ...
  x <- cbind(X[(m+1):n,],P.phi, Q.theta,Q.THETA)#intercepto, covariáveis,ar, ma, MA
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
  ...
  #print("Estimadores")
  #print(opt$par)
  #print("hessiana numérica da estimação analítica")
  #print(opt$hessian)
  #, maxit = maxit1, reltol = 1e-12))
 ...
 # library(rootSolve)
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,]%*%as.matrix(beta) + sum(phi*(ynew[i-ar]#-X[i-ar,]%*%as.matrix(beta)
  ...
  B0 <- matrix(rep(NA,(n-m)*length(beta)),ncol=length(beta))
  for(i in 1:(n-m))
  {
    for(j in 1:length(beta))
    {
      B0[i,j] <- X[i+m,j] #- sum(phi*X[i+m-ar,j]) 
    }
  }
  ...
  K <- rbind(
    cbind(KBB,KBp,KBq,KBQ,KBalpha),
    cbind(KpB,Kpp,Kpq,KpQ,Kpalpha),
    cbind(KqB,Kqp,Kqq,KqQ,Kqalpha),
    cbind(KQB,KQp,KQq,KQQ,KQalpha),
    cbind(KalphaB,Kalphap,Kalphaq,KalphaQ,Kalphaalpha)
  )
  return(K)
  }
  K<-obs.inf(y1,muhat)
  ...
  #print("Teste de Wald sob a hipótese nula de que o estimador é zero",quot=F)
  ...
  z$zsta<-z$coeff/sqrt(v)#Estatísticas Z do Teste de Wald
  #print("Resultado a nível 5%")
  resp<-rep(0,length(z$zsta))
  for (i in 1:length(resp)){
    if(abs(z$zsta[i])>qnorm(0.975))
    {
      resp[i] <- "H0 rejected"
  ...
  z$LI=LI
  z$LS=LS
  z$pvalues<-(1-pnorm(abs(z$zsta)))*2
  #print(pvalue)
  if(is.na(exvar[1,1])==T)
  {
    first_col<-c("intercept",ar, ma, MA,"alpha estimator")
    result <- matrix(c(first_col,round(c(z$coeff,z$zsta,LI,LS,z$pvalues),4),resp), nrow=length(opt$par), ncol=7, byrow=F)
    colnames(result) <- c("Estimator","MLE","Wald's Statistic","Lower bound","Upper bound","p-value","Wald'S Test result")
    rownames(result)<-c("", rep("ar",length(ar)), rep("ma",length(ma)), rep("MA",length(MA)),"")
  }else{
    first_col<-c("intercept", 1:ncol(exvar),ar, ma, MA,"alpha estimator")
    result <- matrix(c(first_col,round(c(z$coeff,z$zsta,LI,LS,z$pvalues),4),resp), nrow=length(opt$par), ncol=7, byrow=F)
    colnames(result) <- c("Estimator","MLE","Wald's Statistic","Lower bound","Upper bound","p-value","Wald'S Test result")
    rownames(result)<-c("", rep("cov",ncol(exvar)), rep("ar",length(ar)), rep("ma",length(ma)), rep("MA",length(MA)),"")
  }
  #print(result,quot=F)
  ...
  rownames(accuracy) <- c("")
     ...
     for(i in m:1)
     {
       etam[i] <- X[i,1]%*%as.matrix(z$beta0) + sum(phi*(ynew[i+ar])) - sum(ma_par*errorm[i+ma_ind])
  ...
  for(i in 1:steps)
  {
    ynew_prev[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) + sum(z$phi*(ynew_prev[n+i-ar]) ) - sum(ma_par*errorhat[n+i-ma_ind])
  ...
  for(i in 1:steps)
  {
    gy_prev[n+i] <- X_prev[n+i,1]*z$beta0  + sum(z$phi*(gy[n+i-ar]) ) - sum(ma_par*errorhat[n+i-ma_ind])
  ...
  ljungbox<- Box.test(residual, lag = aclag, type = "Ljung-Box", fitdf=(p1+q1+Q1))
  boxpierce<-Box.test(residual, lag = aclag, type = c( "Box-Pierce"), fitdf=(p1+q1+Q1))#Box-Pierce test
  monti<- Monti.test(residual, lag = aclag, type = "Ljung-Box", fitdf=(p1+q1+Q1))#Ljung-Box test as the Box.test function using pacf replacing acf
