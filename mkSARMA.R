...
#SARMAX
EMV.mkSARMA <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                        tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,]%*%as.matrix(beta) + sum(PHI*(ynew[i-AR*S] ) ) - sum(THETA*error[i-MA*S])
  ...
  x <- cbind(X[(m+1):n,],P.PHI,Q.THETA)#intercepto, covariáveis,ar, ma, AR, MA
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
 ...
 # opt2 <- optim(reg, loglik, method = "BFGS", hessian = T, control = list(fnscale = -1))
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,]%*%as.matrix(beta) + sum(PHI*(ynew[i-AR*S] ) ) - sum(THETA*errorhat[i-MA*S])
  ...
  for(i in (m+1):n) 
  {
    deta.dbeta0[i,]<- B0[(i-m),] +  THETA%*%deta.dbeta0[i-S*MA,]
   deta.dPHI[i,]<- As[(i-m),] +  THETA%*%deta.dPHI[i-S*MA,]
    deta.dTHETA[i,]<- Rs[(i-m),] +  THETA%*%deta.dTHETA[i-S*MA,]
    deta.dbeta0beta0[i,]<- 0 +  THETA%*%deta.dbeta0beta0[i-S*MA,]
    deta.dbeta0PHI[i,]<- rep(0,P1) + THETA%*%deta.dbeta0PHI[i-S*MA,]
    for(b in 1:Q1)
    {
      deta.dbeta0THETA[i,b] = deta.dbeta0[i-MA[b]*S,]  + THETA%*%deta.dbeta0THETA[i-S*MA,b]
    }
    for(b in 1:P1)
    {
      for(a in 1:P1)
      {
        deta.dPHIPHI[a,b,i]<- 0 +  THETA%*%deta.dPHIPHI[a,b,i-S*MA]
      }
    }
    for(b in 1:Q1)
    {
      for(a in 1:P1)
      {
        deta.dPHITHETA[a,b,i]= deta.dPHI[i-MA[b]*S,a] +THETA%*%deta.dPHITHETA[a,b,i-S*MA]
      }
    }
    for(b in 1:Q1)
    {
      for(a in 1:Q1)
      {
        deta.dTHETATHETA[a,b,i]= deta.dTHETA[i-MA[a]*S,b]+deta.dTHETA[i-MA[b]*S,a]+ THETA%*%deta.dTHETATHETA[a,b,i-S*MA]
      }
    }
  }
  ...
  rownames(accuracy) <- c("")
   ...
   for(i in m:1)
   {
     etam[i] <- X[i,1]%*%as.matrix(z$beta0) + sum(PHI*(ynew[i+S*AR])) - sum(THETA*errorm[i+S*MA])
