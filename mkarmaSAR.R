...
EMV.mkarmaSAR <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                          tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,]%*%as.matrix(beta) + sum(ar_par*(ynew[i-ar_ind]#-X[i-ar_ind,]%*%as.matrix(beta)
      ) ) - sum(theta*error[i-ma])
  ...
  score <- function(z) 
  {
    ...
    for(i in 1:(n-m))
    {
      for(j in 1:p1)
      {
        A[i,j] <- (ynew[i+m-ar[j]]#-X[i+m-j,]%*%as.matrix(beta)
        )-sum(PHI%*%(ynew[i+m-(S*AR+ar[j])]#-X[i+m-(S*AR+j),]%*%as.matrix(beta)
        ))
      }
    }    
    ...
    for(i in 1:(n-m))
    {
      for(j in 1:P1)
      {
        As[i,j] <- (ynew[i+m-(S*AR[j])]#-X[i+m-(S*j),]%*%as.matrix(beta)
        )-sum(phi%*%(ynew[i+m-(S*AR[j]+ar)]#-X[i+m-(S*j+ar),]%*%as.matrix(beta)
        ))
      }
    }    
    ...
    for(i in 1:(n-m))
    {
      ...
      {
        B[i,j] <- X[i+m,j] #- sum(ar_par*X[i+m-ar_ind,j]) 
      }
    }
    ...
    for(i in (m+1):n)
    {
      ...
      deta.dPHI[i,]<- As[(i-m),] +  theta%*%deta.dPHI[i-ma,]
    ...
    return(c(Ubeta,Uphi,Utheta,UPHI,Ualpha))
  }#fim score
 ...
 # opt2 <- optim(reg, loglik, method = "BFGS", hessian = T, control = list(fnscale = -1))
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,]%*%as.matrix(beta) + sum(ar_par*(ynew[i-ar_ind]#-X[i-ar_ind,]%*%as.matrix(beta)
    ) ) - sum(theta*errorhat[i-ma])
  ...
  obs.inf<-function(y1,muhat)
  {
    ...
    z$mustarhat <- muhatstar.sec # - ESPERANÇA DA SEGUNDA DERIVADA DA LOGVEROSSIMILHANÇA EM RELAÇÃO A MU
    ...
    for(i in 1:(n-m))
    {
      for(j in 1:P1)
      {
        As[i,j] <- (ynew[i+m-(S*AR[j])])-sum(phi%*%(ynew[i+m-(S*AR[j]+ar)]
        ))
      }
    }    
    ...
    for(i in 1:(n-m))
    {
      for(b in 1:P1)
      {
        for(a in 1:p1)
        {
          aAs[a,b,i] <- -ynew[i+m-(S*AR[b]+ar[a])]
        }
      }
    }
    ...
    for(i in (m+1):n)
    {
      deta.dbeta0[i,]<-  B[(i-m),] +  theta%*%deta.dbeta0[i-ma,]
      deta.dphi[i,]<- A[(i-m),] +  theta%*%deta.dphi[i-ma,]
      deta.dPHI[i,]<- As[(i-m),] +  theta%*%deta.dPHI[i-ma,]
      deta.dtheta[i,]<- R[(i-m),] +  theta%*%deta.dtheta[i-ma,]
    ...
    for(b in 1:P1)
    {
      for(a in 1:p1)
      {
        deta.dphiPHI[a,b,i]<- aAs[a,b,(i-m)] +  theta%*%deta.dphiPHI[a,b,i-ma]
      }
    }
    for(b in 1:q1)
    {
    ...
    K <- rbind(
      cbind(KB0B,KB0p,KB0q,KB0P,KB0alpha),
      cbind(KpB0,Kpp,Kpq,KpP,Kpalpha),
      cbind(KqB0,Kqp,Kqq,KqP,Kqalpha),
      cbind(KPB0,KPp,KPq,KPP,KPalpha),
      cbind(KalphaB0,Kalphap,Kalphaq,KalphaP,Kalphaalpha)
    )
    return(K)
  }
  K<-obs.inf(y1,muhat)
  ...
  {
    first_col<-c("intercept",ar, ma, AR,"alpha estimator")
    ...
    rownames(result)<-c("", rep("ar",length(ar)), rep("ma",length(ma)), rep("AR",length(AR)),"")
  }else{
    first_col<-c("intercept", 1:ncol(exvar),ar, ma, AR,"alpha estimator")
    result <- matrix(c(first_col,round(c(z$coeff,z$zstat,LI,LS,z$pvalues),4),resp), nrow=length(opt$par), ncol=7, byrow=F)
    colnames(result) <- c("Estimator","MLE","Wald's Statistic","Lower bound","Upper bound","p-value","Wald'S Test result")
    rownames(result)<-c("", rep("cov",ncol(exvar)), rep("ar",length(ar)), rep("ma",length(ma)), rep("AR",length(AR)),"")
  }
  #print(result,quot=F)
  ...
  for(i in m:1)
  {
    etam[i] <- X[i,1]%*%as.matrix(z$beta0) + sum(ar_par*(ynew[i+ar_ind])) - sum(theta*errorm[i+ma])
  ...
  if(steps!=0){
    ...
    for(i in 1:steps)
    {
      ynew_prev[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) + sum(ar_par*(ynew_prev[n+i-ar_ind] )) - sum(z$theta*errorhat[n+i-ma])
