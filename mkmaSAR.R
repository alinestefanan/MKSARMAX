...
EMV.mkmaSAR <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                        tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,]%*%as.matrix(beta) + sum(PHI*(ynew[i-AR*S] ) ) - sum(theta*error[i-ma])
  ...
  score <- function(z) 
  {
    ...
    return(c(Ubeta,Utheta,UPHI,Ualpha))
  }#fim score
  ...
  x <- cbind(X[(m+1):n,], Q.theta ,P.PHI)#intercepto, covariáveis, ma, AR
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
...
#  print(rbind(score(opt$par),gradient(loglik,opt$par)))
 # print(rbind(score(opt2$par),gradient(loglik,opt2$par)))
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,]%*%as.matrix(beta) + sum(PHI*(ynew[i-AR*S] ) ) - sum(theta*errorhat[i-ma])
  ...
  K <- rbind(
    cbind(KBB,KBq,KBP,KBalpha),
    cbind(KqB,Kqq,KqP,Kqalpha),
    cbind(KPB,KPq,KPP,KPalpha),
    cbind(KalphaB,Kalphaq,KalphaP,Kalphaalpha)
  )
  return(K)
  }
  K<-obs.inf(y1,muhat)
  ...
  {
    first_col<-c("intercept", ma, AR,"alpha estimator")
    ...
    rownames(result)<-c("", rep("ma",length(ma)), rep("AR",length(AR)),"")
  }else{
    first_col<-c("intercept", 1:ncol(exvar), ma, AR,"alpha estimator")
    result <- matrix(c(first_col,round(c(z$coeff,z$zstat,LI,LS,z$pvalues),4),resp), nrow=length(opt$par), ncol=7, byrow=F)
    colnames(result) <- c("Estimator","MLE","Wald's Statistic","Lower bound","Upper bound","p-value","Wald'S Test result")
    rownames(result)<-c("", rep("cov",ncol(exvar)), rep("ma",length(ma)), rep("AR",length(AR)),"")
  }
  #print(result,quot=F)
  ...
  for(i in m:1)
  {
    etam[i] <- X[i,1]%*%as.matrix(z$beta0) + sum(PHI*(ynew[i+S*AR])) - sum(theta*errorm[i+ma])
  ...
  for(i in 1:steps)
  {
    ynew_prev[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) + sum(z$Phi*(ynew_prev[n+i-AR*S]) ) - sum(z$theta*errorhat[n+i-ma])
  ...
  for(i in 1:steps)
  {
    gy_prev[n+i] <- X_prev[n+i,1]*z$beta0  + sum(z$Phi*(gy[n+i-AR*S]) ) - sum(z$theta*errorhat[n+i-ma])
