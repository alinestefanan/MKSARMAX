...
EMV.mkarX <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=NA,
                             tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,1]*beta0 + X[i,2:ncol(X)]%*%as.matrix(beta) + sum(phi*(ynew[i-ar]-X[i-ar,2:ncol(X)]%*%as.matrix(beta)  ) ) 
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,1]*beta0 + X[i,2:ncol(X)]%*%as.matrix(beta) + sum(phi*(ynew[i-ar]-X[i-ar,2:ncol(X)]%*%as.matrix(beta)) ) 
  ...
  for(i in m:1)
  {
    etam[i] <- X[i,1]*z$beta0 +X[i,2:ncol(X)]%*%as.matrix(z$beta)+ sum(phi*(ynew[i+ar])-X[i+ar,2:ncol(X)]%*%as.matrix(z$beta))
  ...
  for(i in 1:steps)
  {
    ynew_prev[n+i] <- X_prev[n+i,1]*z$beta0 + X_prev[n+i,2:ncol(X_prev)]%*%as.matrix(z$beta) + sum(phi*(ynew_prev[n+i-ar]-X_prev[n+i-ar,2:ncol(X_prev)]%*%as.matrix(z$beta)
    ) ) 
