...
EMV.mkarSARMA <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                          tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    beta <- z[1:(k+1)]
    phi = z[(k+2):(p1+k+1)] 
    PHI = z[(p1+k+2):(p1+P1+k+1)]
    THETA = z[(p1+P1+k+2):(p1+P1+Q1+k+1)]
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,]%*%as.matrix(beta) + sum(ar_par*(ynew[i-ar_ind]) ) - sum(THETA*error[i-MA*S])
  ...
  score <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      deta.dbeta[i,]<-  B[(i-m),] +  THETA%*%deta.dbeta[i-MA*S,]
      
      deta.dphi[i,]<- A[(i-m),] +  THETA%*%deta.dphi[i-MA*S,]
      
      deta.dPHI[i,]<- As[(i-m),] +  THETA%*%deta.dPHI[i-MA*S,]
      
      deta.dTHETA[i,]<- Rs[(i-m),] + THETA%*%deta.dTHETA[i-MA*S,]
    }
    ...
    return(c(Ubeta,Uphi,UPHI,UTHETA,Ualpha))
  }#fim score
  ...
  x <- cbind(X[(m+1):n,],P.phi ,P.PHI,Q.THETA)#intercepto, covariáveis,ar, AR, MA
  ...
  mqo = c(ajuste$coef)
  for(i in 1:length(mqo))#porque no MA vai virar NA, para então deixar 0
  {
    if (is.na(mqo[i])){
      mqo[i] <- 0
    }
 ...
 # library(rootSolve)
  ...
  PHI <-coef[(p1+k+2):(p1+P1+k+1)] #AR
  THETA <-coef[(p1+P1+k+2):(p1+P1+Q1+k+1)] #MA
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,]%*%as.matrix(beta) + sum(ar_par*(ynew[i-ar_ind]   ) ) - sum(THETA*errorhat[i-MA*S])
  ...
  obs.inf<-function(y1,muhat)
  {
    ...
    for(i in 1:(n-m))
    {
      for(j in 1:p1)
      {
        A[i,j] <- (ynew[i+m-ar[j]])-(PHI%*%(ynew[i+m-(S*AR+ar[j])] ))
      }
    }    
    ...
    for(i in (m+1):n)
    {
      deta.dbeta0[i,]<-  B0[(i-m),] +  THETA%*%deta.dbeta0[i-MA*S,]
      deta.dphi[i,]<- A[(i-m),] +  THETA%*%deta.dphi[i-MA*S,]
      deta.dPHI[i,]<- As[(i-m),] +  THETA%*%deta.dPHI[i-MA*S,]
      deta.dTHETA[i,]<- Rs[(i-m),] + THETA%*%deta.dTHETA[i-MA*S,]
      deta.dbeta0beta0[i,]<- 0 +  THETA%*%deta.dbeta0beta0[i-MA*S,]
      deta.dbeta0phi[i,]<- rep(0,p1) +  THETA%*%deta.dbeta0phi[i-MA*S,]
      deta.dbeta0PHI[i,]<- rep(0,P1) +  THETA%*%deta.dbeta0PHI[i-MA*S,]
      for(b in 1:Q1)
      {
        deta.dbeta0THETA[i,b] = deta.dbeta0[i-MA[b]*S,] + THETA%*%deta.dbeta0THETA[i-MA*S,b]
      }
      for(b in 1:p1)
      {
        for(a in 1:p1)
        {
          deta.dphiphi[a,b,i]<- 0 +  THETA%*%deta.dphiphi[a,b,i-MA*S]
        }
      }
      for(b in 1:P1)
      {
        for(a in 1:p1)
        {
          deta.dphiPHI[a,b,i]<- aAs[a,b,(i-m)] +  THETA%*%deta.dphiPHI[a,b,i-MA*S]
        }
      }
      for(b in 1:Q1)
      {
        for(a in 1:p1)
        {
          deta.dphiTHETA[a,b,i]<- deta.dphi[i-MA[b]*S,a] +  THETA%*%deta.dphiTHETA[a,b,i-MA*S]
        }
      }
      for(b in 1:P1)
      {
      ...
      for(b in 1:Q1)
      {
        for(a in 1:Q1)
        {
          deta.dTHETATHETA[a,b,i]= deta.dTHETA[i-MA[a]*S,b]+deta.dTHETA[i-MA[b]*S,a]+ THETA%*%deta.dTHETATHETA[a,b,i-MA*S]
        }
      }    }
    ...
    K <- rbind(
      cbind(KBB,KBp,KBP,KBQ,KBalpha),
      cbind(KpB,Kpp,KpP,KpQ,Kpalpha),
      cbind(KPB,KPp,KPP,KPQ,KPalpha),
      cbind(KQB,KQp,KQP,KQQ,KQalpha),
      cbind(KalphaB,Kalphap,KalphaP,KalphaQ,Kalphaalpha)
    )
    return(K)
  }
  K<-obs.inf(y1,muhat)
  ...
  {
    first_col<-c("intercept",ar, AR, MA,"alpha estimator")
    ...
    first_col<-c("intercept", 1:ncol(exvar),ar, AR, MA,"alpha estimator")
    result <- matrix(c(first_col,round(c(z$coeff,z$zstat,LI,LS,z$pvalues),4),resp), nrow=length(opt$par), ncol=7, byrow=F)
    colnames(result) <- c("Estimator","MLE","Wald's Statistic","Lower bound","Upper bound","p-value","Wald'S Test result")
    rownames(result)<-c("", rep("cov",ncol(exvar)), rep("ar",length(ar)), rep("AR",length(AR)), rep("MA",length(MA)),"")
  }
 ...
 # histogram<-recordPlot()
  ...
  for(i in m:1)
  {
    etam[i] <- X[i,1]%*%as.matrix(z$beta0) + sum(ar_par*(ynew[i+ar_ind])) - sum(THETA*errorm[i+MA*S])
  ...
  for(i in 1:steps)
  {
    ynew_prev[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) + sum(ar_par*(ynew_prev[n+i-ar_ind]) ) - sum(z$Theta*errorhat[n+i-MA*S])
  ...
  for(i in 1:steps)
  {
    gy_prev[n+i] <- X_prev[n+i,1]*z$beta0  + sum(ar_par*(gy[n+i-ar_ind]) ) - sum(z$Theta*errorhat[n+i-MA*S])
  ...
  plot(z$resid1,xlab="n",ylab="Quantile residuals")
  quantile_resid<-recordPlot()
  
  acf(z$resid1)
  acf_quantile_resid<-recordPlot()
  ...
  if(resid==1) residual <- z$resid1
  if(resid==2) residual <- z$resid2
  z$residual=residual
  z$deviance <- 2*sum(dt)
  ...
  monti<- Monti.test(residual, lag = aclag, type = "Ljung-Box", fitdf=(p1+P1+Q1))#Ljung-Box test as the Box.test function using pacf replacing acf
