...
EMV.mkSMAX <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                       tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  score <- function(z) 
  {
    ...
    ##############################################################################
    return(c(Ubeta0,Ubeta,UTHETA,Ualpha))
  }#end score
  ...
  x <- cbind(X[(m+1):n,], Q.THETA)#intercepto, covariáveis, ma, MA
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
  ...
  obs.inf<-function(y1,muhat)
  {
    ...
    for(i in (m+1):n)
    {
      ...
      for(b in 1:Q1)
      {
        deta.dbeta0THETA[i,b] = deta.dbeta0[i-MA[b]*S,] + sum(ma_par%*%deta.dbeta0THETA[i-ma_ind,b])
      }
      for(b in 1:k)
      {
        for(a in 1:k)
      ...
      for(b in 1:Q1)
      {
        for(a in 1:k)
        {
          deta.dbetaTHETA[a,b,i] <- deta.dbeta[i-MA[b]*S,a]  + sum(ma_par%*%deta.dbetaTHETA[a,b,i-ma_ind])
        }
      }
      for(b in 1:Q1)
      {
        for(a in 1:Q1)
        {
          deta.dTHETATHETA[a,b,i]= deta.dTHETA[i-MA[a]*S,b]+deta.dTHETA[i-MA[b]*S,a]+ sum(ma_par%*%deta.dTHETATHETA[a,b,i-ma_ind])
        }
      }
    }
    ...
    K <- rbind(
      cbind(KB0B0,KB0B,KB0Q,KB0alpha),
      cbind(KBB0,KBB,KBQ,KBalpha),
      cbind(KQB0,KQB,KQQ,KQalpha),
      cbind(KalphaB0,KalphaB,KalphaQ,Kalphaalpha)
    )
    return(K)
  }
  K<-obs.inf(y1,muhat)
  ...
  first_col<-c("intercept", 1:k, MA,"alpha estimator")
  ...
  rownames(result)<-c("", rep("cov",k), rep("MA",length(MA)),"")
  ...
  z$deviance <- 2*sum(dt)
  z$dof.dev=(n-m-Q1)#desconsidera intercepto do eta e alpha da distribuição
