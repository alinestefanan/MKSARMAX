...
EMV.mkarmaSARMA <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,
                            tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  X0<-matrix(rep(1,n), nrow=n, ncol=1, byrow=F) #default
  ...
  dmk <- Vectorize(function(y,alpha,beta,log = FALSE){
    ...
    for (i in 1:length(density)){
      ...
      #if(density[i]>0.9999999) density[i]<-0.9999999
    }
    logden <- log(density)
  ...
  loglik <- function(z) 
  {
    ...
    alpha <- z[(p1+P1+q1+Q1+2)] # alpha parameter
   error<-rep(0,n) # E(error)=0 
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X0[i,]%*%as.matrix(beta0) + sum(ar_par*(ynew[i-ar_ind] ) ) - sum(ma_par*error[i-ma_ind])
  ...
  score <- function(z) 
  {
    ...
    n1=alpha*e.a*(den.cr+log(0.5)*critical.ly)
    d1=(mu^2)*(exp(alpha/mu)-e.a)*(den.cr^2)
    dmu=as.vector(n1/d1)
    ...
    R <- matrix(rep(NA,n*q1),ncol=q1)# theta
    for(i in (m+1):n)
    {
      for(j in 1:q1)
      {
        R[i,j] <- -sum(error[i-c(ma[j],ma[j]+(S*MA))]*c(1,-THETA))
      }
    }
    ...
    for(i in (m+1):n)
    {
      for(j in 1:Q1)
      {
        Rs[i,j] <- -sum(error[i-c(S*MA[j],S*MA[j]+(ma))]*c(1,-theta))
      }
    }
    
    A <- matrix(rep(NA,n*(p1)),ncol=(p1))# ar
    for(i in (m+1):n)
    {
      for(j in 1:p1)
      {
        A[i,j] <- (ynew[i-ar[j]] )-sum(PHI%*%(ynew[i-(S*AR+ar[j])]  ))
      }
    }    
    ...
    B0 <- matrix(rep(NA,n),ncol=1) #intercept
    for(i in (m+1):n)
    {
    ...
    for(i in (m+1):n)
    {
      deta.dbeta0[i,]<-  B0[i,] +  ma_par%*%deta.dbeta0[i-ma_ind,]
      
      deta.dphi[i,]<- A[i,] +  ma_par%*%deta.dphi[i-ma_ind,]
      
      deta.dPHI[i,]<- As[i,] +  ma_par%*%deta.dPHI[i-ma_ind,]
      
      deta.dtheta[i,]<- R[i,] +  ma_par%*%deta.dtheta[i-ma_ind,]
      
      deta.dTHETA[i,]<- Rs[i,] +  ma_par%*%deta.dTHETA[i-ma_ind,]
    }
    ...
    return(c(Ubeta0,Uphi,Utheta,UPHI,UTHETA,Ualpha))
  }#end score
  ...
  x <- cbind(X0[(m+1):n,],P.phi, Q.theta ,P.PHI,Q.THETA)#intercepto,ar, ma, AR, MA
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
 ...
 # tic("estimacao tempo")
  ...
  # opt.error<- tryCatch(optimParallel(reg, loglik, score,
  #                            method = "L-BFGS-B",
  #                            control = list(fnscale = -1,maxit=400,pgtol=1e-6,factr=1e20)), error = function(e) return("error"))
  # if(opt.error[1] == "error")
  # {z$RMC=1
  # #stop("optim error")
  # warning("optim error")
  # return(z)
  # }
  # opt <- optimParallel(reg, loglik, score,
  #              method = "L-BFGS-B", hessian = T,
  #              control = list(fnscale = -1,maxit=400,
  #                             pgtol=1e-6,factr=1e20))
#  toc()
  
  
 # opt2 <- optim(reg, loglik, method = "BFGS", hessian = T, control = list(fnscale = -1))#, maxit = maxit1, reltol = 1e-12))
  #opt<-opt2 # -  # -  # -   #  - # -  # -  # -   #  - tirar depois de simulacao # -  # -  # -   #  - # -  # -  # -   #  - # -  # -  # -   #  - # -  # -  # -   #  -
  ...
  alpha <-coef[(p1+P1+q1+Q1+2)] # alpha parameter
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X0[i,]%*%as.matrix(beta0) + sum(ar_par*(ynew[i-ar_ind] ) ) - sum(ma_par*errorhat[i-ma_ind])
    ...
    obs.inf<-function(y1,muhat)
  {
    ...
    #z$mustarhat <- muhatstar.sec # EXPECTED VALUE OF SECOND DERIVATIVE FROM LOGLIKELIHOOD IN RESPECT TO MU, IF FISHER INFORMATION MATRIX
    ...
    for(i in (m+1):n)
    {
      for(j in 1:q1)
      {
        R[i,j] <- -sum(errorhat[i-c(ma[j],ma[j]+(S*MA))]*c(1,-THETA))
      }
    }
    Rs <- matrix(rep(NA,n*Q1),ncol=Q1)# THETA SMA
    for(i in (m+1):n)
    ...
    deta.dPHITHETA<-array(0,dim=c(P1,Q1,n))
    deta.dTHETATHETA<-array(0,dim=c(Q1,Q1,n))
    #print("ma_par%*%deta.dbeta0theta[m+15-ma_ind,1]");print(ma_par%*%deta.dbeta0theta[m+15-ma_ind,1])
    for(i in (m+1):n) 
    {
      deta.dbeta0[i,]<- B0[i,] +  ma_par%*%deta.dbeta0[i-ma_ind,]
      deta.dphi[i,]<- A[i,] +  ma_par%*%deta.dphi[i-ma_ind,]
      ...
      for(a in 1:q1)
      {
        deta.dbeta0theta[i,a] = deta.dbeta0[i-ma[a],] -sum(THETA%*%deta.dbeta0[i-(ma[a]+MA*S),]) +sum(ma_par%*%deta.dbeta0theta[i-ma_ind,a])
      }
      deta.dbeta0PHI[i,]<- rep(0,P1) +  ma_par%*%deta.dbeta0PHI[i-ma_ind,]
      for(b in 1:Q1)
      {
        deta.dbeta0THETA[i,b] = deta.dbeta0[i-MA[b]*S,] -sum(theta%*%deta.dbeta0[i-(ma+MA[b]*S),]) + sum(ma_par%*%deta.dbeta0THETA[i-ma_ind,b])
      }
      for(b in 1:p1)
      {
        for(a in 1:p1)
        {
          deta.dphiphi[a,b,i]<- 0 + sum(ma_par%*%deta.dphiphi[a,b,i-ma_ind])
        }
      }
      for(b in 1:q1)
      {
        for(a in 1:p1)
        {
          deta.dphitheta[a,b,i]= deta.dphi[i-ma[b],a] -sum(THETA%*%deta.dphi[i-(ma[b]+MA*S),a])+sum(ma_par%*%deta.dphitheta[a,b,i-ma_ind])
        }
      }
      for(b in 1:P1)
      {
        for(a in 1:p1)
        {
          deta.dphiPHI[a,b,i]<- aAs[a,b,i] +  ma_par%*%deta.dphiPHI[a,b,i-ma_ind]
        }
      }
      for(b in 1:Q1)
      {
        for(a in 1:p1)
        {
          deta.dphiTHETA[a,b,i]<- deta.dphi[i-MA[b]*S,a] -sum(theta%*%deta.dphi[i-(ma+MA[b]*S),a]) +  sum(ma_par%*%deta.dphiTHETA[a,b,i-ma_ind])
        }
      }
      for(b in 1:q1)
      {
        for(a in 1:q1)
        {
          deta.dthetatheta[a,b,i]= deta.dtheta[i-ma[a],b] -sum(THETA%*%deta.dtheta[i-(ma[a]+MA*S),b])+deta.dtheta[i-ma[b],a]-sum(THETA%*%deta.dtheta[i-(ma[b]+MA*S),a])+sum(ma_par%*%deta.dthetatheta[a,b,i-ma_ind])
        }
      }
      for(b in 1:Q1)
      {
        for(a in 1:q1)
        {
          deta.dthetaTHETA[a,b,i]= deta.dtheta[i-MA[b]*S,a]+RRs[a,b,i]-sum(theta%*%deta.dtheta[i-(ma+MA[b]*S),a])+deta.dTHETA[i-ma[a],b]-sum(THETA%*%deta.dTHETA[i-(ma[a]+MA*S),b])+sum(ma_par%*%deta.dthetaTHETA[a,b,i-ma_ind])
        }
      }
      for(b in 1:P1)
      {
      ...
      for(b in 1:Q1)
      {
        for(a in 1:Q1)
        {
          deta.dTHETATHETA[a,b,i]= deta.dTHETA[i-MA[a]*S,b]-sum(theta%*%deta.dTHETA[i-(ma+MA[a]*S),b])+deta.dTHETA[i-MA[b]*S,a]-sum(theta%*%deta.dTHETA[i-(ma+MA[b]*S),a])+ sum(ma_par%*%deta.dTHETATHETA[a,b,i-ma_ind])
        }
      }
    }
    ...
    KpP=matrix(rep(NA,p1*P1),ncol=P1)
    if(length(KpP)==1){
      KpP<- -(t(pp)%*%mV%*%(mT^2)%*%PP+t(pp)%*%mV0%*%mT2%*%PP + t(vI)%*%mV0%*%mT%*%pP)
    }else{
      for(j in 1:P1){
        for(i in 1:p1){
    ...
    K <- rbind(
      cbind(KBB,KBp,KBq,KBP,KBQ,KBalpha),
      cbind(KpB,Kpp,Kpq,KpP,KpQ,Kpalpha),
      cbind(KqB,Kqp,Kqq,KqP,KqQ,Kqalpha),
      cbind(KPB,KPp,KPq,KPP,KPQ,KPalpha),
      cbind(KQB,KQp,KQq,KQP,KQQ,KQalpha),
      cbind(KalphaB,Kalphap,Kalphaq,KalphaP,KalphaQ,Kalphaalpha)
    )
    return(K)
  }
  K<-obs.inf(y1,muhat)
  ...
  #print(round( 100*(K+opt2$hessian)/K,2))
  #print(round( 100*(K+opt2$hessian)/K,2))
  ...
  first_col<-c("intercept",ar, ma, AR, MA,"alpha estimator")
  ...
  for(i in m:1)
  {
    etam[i] <- X0[i,1]%*%as.matrix(beta0) + sum(ar_par*(ynewm[i+ar_ind])) - sum(ma_par*errorm[i+ma_ind])
  ...
  for(i in m:1)
  {
    etam[i] <- X0[i,1]%*%as.matrix(beta0) + sum(ar_par*(ynew[i+ar_ind])) - sum(ma_par*errorm[i+ma_ind])
  ...
  if(steps!=0){
    ...
    X0_prev<-matrix(rep(1,(n+steps)), nrow=(n+steps), ncol=1, byrow=F)
    
    ntotal<-n+steps
    
    for(i in 1:steps)
    {
      ynew_prev[n+i] <- X0_prev[n+i,]%*%as.matrix(z$beta0) + sum(ar_par*(ynew_prev[n+i-ar_ind] ) ) - sum(ma_par*errorhat[n+i-ma_ind])
    ...
    for(i in 1:steps)
    {
      gy_prev[n+i] <- X0_prev[n+i,1]*z$beta0  + sum(ar_par*(gy[n+i-ar_ind]) ) - sum(ma_par*errorhat[n+i-ma_ind])
  ...
  if(graph==T)
  {
    ...
    if(steps!=0){
      ...
      dev.off()
      pdf("fitted.all.forecast.pdf",width=5, height=4)
      {
        par(mfrow=c(1,1))
        par(mar=c(2.8, 2.7, 1, 1)) # margens c(baixo,esq,cima,direia)
        par(mgp=c(1.7, 0.45, 0))
        plot(as.vector(y),type="l",ylab="Serie",xlab="Time",ylim=c(0,1))
        abline(v=m,lty=2)
        abline(v=n,lty=2)
        lines(c(z$fit.all,y_prev[(n+1):(n+steps)]),col="blue",lty=2)
        legend("bottomleft",c("Observed data","Fitted and forecast values"),#pch=vpch,
               pt.bg="white", lty=c(1,2), bty="n",col=c(1,"blue"))
      }
      dev.off()
  ...
  monti<- Monti.test(residual, lag = aclag, type = "Ljung-Box", fitdf = (p1+q1+P1+Q1))#Ljung-Box test as the Box.test function using pacf replacing acf
