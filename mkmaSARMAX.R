...
EMV.mkmaSARMAX <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                           tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,1]*beta0 + X[i,2:ncol(X)]%*%as.matrix(beta) + sum(ar_par*(ynew[i-ar_ind]-X[i-ar_ind,2:ncol(X)]%*%as.matrix(beta)  ) ) - sum(ma_par*error[i-ma_ind])
  ...
  score <- function(z) 
  {
    ...
    for(i in 1:(n-m))
    {
      for(j in 1:P1)
      {
        As[i,j] <- (ynew[i+m-(S*AR[j])]-X[i+m-(S*AR[j]),2:ncol(X)]%*%as.matrix(beta)
        )
      }
    }    
    ...
    for(i in 1:(n-m))
    {
      for(j in 1:k)
      {
        B[i,j] <- X[i+m,1+j] - sum(ar_par*X[i+m-ar_ind,1+j])
      }
    }
    ...
    ##############################################################################
    return(c(Ubeta0,Ubeta,Utheta,UPHI,UTHETA,Ualpha))
  }#end score
  ...
  x <- cbind(X[(m+1):n,], Q.theta ,P.PHI,Q.THETA)#intercepto, covariáveis,ar, ma, AR, MA
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,1]*beta0 + X[i,2:ncol(X)]%*%as.matrix(beta) + sum(ar_par*(ynew[i-ar_ind]-X[i-ar_ind,2:ncol(X)]%*%as.matrix(beta)) ) - sum(ma_par*errorhat[i-ma_ind])
  ...
  obs.inf<-function(y1,muhat)
  {
    ...
    for(i in (m+1):n)
    {
      ...
      deta.dbeta0beta[i,]<-rep(0,k) +  ma_par%*%deta.dbeta0beta[i-ma_ind,]
      for(a in 1:q1)
      {
      ...
      for(b in 1:k)
      {
        for(a in 1:k)
        {
          deta.dbetabeta[a,b,i] <- 0 + ma_par%*%deta.dbetabeta[a,b,i-ma_ind]
        }
      }
      for(b in 1:q1)
      {
        for(a in 1:k)
        {
          deta.dbetatheta[a,b,i] <- deta.dbeta[i-ma[b],a] - sum(THETA%*%deta.dbeta[i-(ma[b]+MA*S),a]) + ma_par%*%deta.dbetatheta[a,b,i-ma_ind]
        }
      }
      for(b in 1:P1)
      {
        for(a in 1:k)
        {
          deta.dbetaPHI[a,b,i]<- BAs[a,b,(i-m)] + ma_par%*%deta.dbetaPHI[a,b,i-ma_ind]
        }
      }
      for(b in 1:Q1)
      {
        for(a in 1:k)
        {
          deta.dbetaTHETA[a,b,i] <- deta.dbeta[i-MA[b]*S,a] -sum(theta%*%deta.dbeta[i-(ma+MA[b]*S),a]) + ma_par%*%deta.dbetaTHETA[a,b,i-ma_ind]
        }
      }
      for(b in 1:q1)
      {
    ...
    K <- rbind(
      cbind(KB0B0,KB0B,KB0q,KB0P,KB0Q,KB0alpha),
      cbind(KBB0,KBB,KBq,KBP,KBQ,KBalpha),
      cbind(KqB0,KqB,Kqq,KqP,KqQ,Kqalpha),
      cbind(KPB0,KPB,KPq,KPP,KPQ,KPalpha),
      cbind(KQB0,KQB,KQq,KQP,KQQ,KQalpha),
      cbind(KalphaB0,KalphaB,Kalphaq,KalphaP,KalphaQ,Kalphaalpha)
    )
    return(K)
  }
  K<-obs.inf(y1,muhat)
  ...
  first_col<-c("intercept", 1:k, ma, AR, MA,"alpha estimator")
  ...
  rownames(result)<-c("", rep("cov",k),  rep("ma",length(ma)), rep("AR",length(AR)), rep("MA",length(MA)),"")
  ...
  # for(i in m:1)
  # {
  #   etam[i] <- X[i,1]*z$beta0 +X[i,2:ncol(X)]%*%as.matrix(z$beta) + sum(PHI*(ynew[i+S*AR])-X[i+S*AR,2:ncol(X)]%*%as.matrix(z$beta)) - sum(ma_par*errorm[i+ma_ind])
  ...
  if(steps!=0){
    ...
    for(i in 1:steps)
    {
      ynew_prev[n+i] <- X_prev[n+i,1]*z$beta0 + X_prev[n+i,2:ncol(X_prev)]%*%as.matrix(z$beta) + sum(ar_par*(ynew_prev[n+i-ar_ind]-X_prev[n+i-ar_ind,2:ncol(X_prev)]%*%as.matrix(z$beta)
  ...
  z$deviance <- 2*sum(dt)
  z$dof.dev=(n-m-q1-P1-Q1)#desconsidera intercepto do eta e alpha da distribuição
