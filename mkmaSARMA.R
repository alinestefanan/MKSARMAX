...
EMV.mkmaSARMA <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                          tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=F,print=T,check=F,link="logit")
{
  k<-0 #default
  if(validation==T){
    n <- length(y)-steps 
  ...
  if(is.na(exvar[1,1])==F)
  {
    k <- ncol(exvar)
    X <- matrix(c(rep(1,nrow(exvar)),exvar), nrow=nrow(exvar), ncol=(k+1), byrow=F)
  }
  ...
  dmk_alpha<-function(alpha){
    ...
    density<-(alpha*beta*exp(alpha-alpha/y1)*(1-exp(alpha-alpha/y1))^(beta-1))   /(y1^2)
    density[is.na(density)] <- .Machine$double.eps
    density[is.nan(density)] <- .Machine$double.eps
    for (i in 1:length(density)){
      if(density[i]<.Machine$double.eps) density[i]<-.Machine$double.eps
  ...
  P <- max(AR)
  Q <- max(MA)
  m <- max(q,P,Q,S*P,S*Q,S*Q+q,na.rm=T)
  q1 <- length(ma)
  P1 <- length(AR)
  Q1 <- length(MA)
  ...
  loglik <- function(z) 
  {
    beta <- z[1:(k+1)]
    theta =z[(k+2):(q1+k+1)]
    PHI = z[(q1+k+2):(q1+P1+k+1)]
    THETA = z[(P1+q1+k+2):(P1+q1+Q1+k+1)]
    alpha <- z[(P1+q1+Q1+k+2)] # alpha parameter
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,]%*%as.matrix(beta) + sum(PHI*(ynew[i-AR*S]) ) - sum(ma_par*error[i-ma_ind])
  ...
  score <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,]%*%as.matrix(beta) + sum(PHI*(ynew[i-AR*S]#-X[i-AR*S,]%*%as.matrix(beta)
                                                   ) ) - sum(ma_par*error[i-ma_ind])
      error[i] <- ynew[i]-eta[i] #residuals on the predictor scale
    }
    ...
    for(i in 1:(n-m))
    {
      for(j in 1:P1)
      {
        As[i,j] <- (ynew[i+m-(S*AR[j])])
      }
    }    
    
    B <- matrix(rep(NA,(n-m)*length(beta)),ncol=length(beta))#beta
    for(i in 1:(n-m))
    {
    ...
    deta.dbeta <- matrix(0,ncol=k+1,nrow=n)
    ...
    for(i in (m+1):n)
    {
      deta.dbeta[i,]<-  B[(i-m),] +  ma_par%*%deta.dbeta[i-ma_ind,]
      
      deta.dPHI[i,]<- As[(i-m),] +  ma_par%*%deta.dPHI[i-ma_ind,]
      
      deta.dtheta[i,]<- R[(i-m),] +  ma_par%*%deta.dtheta[i-ma_ind,]
      
      deta.dTHETA[i,]<- Rs[(i-m),] +  ma_par%*%deta.dTHETA[i-ma_ind,]
    }
    ...
    return(c(Ubeta,Utheta,UPHI,UTHETA,Ualpha))
  }#fim score
  
  # valores iniciais estimação
  
  P.PHI <- matrix(rep(NA,(n-m)*P1),ncol=P1) #AR
  for(i in 1:(n-m))
  {
    P.PHI[i,] <- ynew[i+m-(S*AR)]
  }
  ...
  x <- cbind(X[(m+1):n,], Q.theta ,P.PHI,Q.THETA)#intercepto, covariáveis, ma, AR, MA
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
  ...
  #print("estimação numérica")
  #print(opt2$par)
  #print("hessiana numérica da estimação numérica")
  #print(opt2$hessian)
  ...
  z$conv <- opt$conv
  coef <- (opt$par)[1:(P1+q1+Q1+k+2)]
  z$coeff <- coef
  
  beta <-coef[1:(k+1)] #intercept and covariates
  theta <-coef[(k+2):(q1+k+1)] #ma
  PHI <-coef[(q1+k+2):(q1+P1+k+1)] #AR
  THETA <-coef[(P1+q1+k+2):(P1+q1+Q1+k+1)] #MA
  alpha <-coef[(P1+q1+Q1+k+2)] # alpha parameter
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,]%*%as.matrix(beta) + sum(PHI*(ynew[i-AR*S]#-X[i-AR*S,]%*%as.matrix(beta)
                                                    ) ) - sum(ma_par*errorhat[i-ma_ind])
  ...
  B0 <- matrix(rep(NA,(n-m)*length(beta)),ncol=length(beta))#beta
  for(i in 1:(n-m))
  {
    for(j in 1:length(beta))
    {
  ...
  RRs <- array(NA,c(q1,Q1,(n-m)))# theta(THETA SMA)
  for(i in 1:(n-m))
  {
    for(b in 1:Q1)
    {
      for(a in 1:q1)
      {
        RRs[a,b,i] <- errorhat[i+m-(S*MA[b]+ma[a])]
      }
    }
  }
  ...
  for(i in (m+1):n)
  {
    deta.dbeta0[i,]<-  B0[(i-m),] +  ma_par%*%deta.dbeta0[i-ma_ind,]
    deta.dPHI[i,]<- As[(i-m),] +  ma_par%*%deta.dPHI[i-ma_ind,]
    deta.dtheta[i,]<- R[(i-m),] +  ma_par%*%deta.dtheta[i-ma_ind,]
    deta.dTHETA[i,]<- Rs[(i-m),] +  ma_par%*%deta.dTHETA[i-ma_ind,]
    deta.dbeta0beta0[i,]<- 0 +  ma_par%*%deta.dbeta0beta0[i-ma_ind,]
    for(a in 1:q1)
    {
      deta.dbeta0theta[i,a] = deta.dbeta0[i-ma[a],] -THETA%*%deta.dbeta0[i-(ma[a]+MA*S),] +ma_par%*%deta.dbeta0theta[i-ma_ind,a]
    }
    deta.dbeta0PHI[i,]<- rep(0,P1) +  ma_par%*%deta.dbeta0PHI[i-ma_ind,]
    for(b in 1:Q1)
    {
      deta.dbeta0THETA[i,b] = deta.dbeta0[i-MA[b]*S,] -theta%*%deta.dbeta0[i-(ma+MA[b]*S),] + ma_par%*%deta.dbeta0THETA[i-ma_ind,b]
    }
    for(b in 1:q1)
    {
      for(a in 1:q1)
      {
        deta.dthetatheta[a,b,i]= deta.dtheta[i-ma[a],b] -sum(THETA%*%deta.dtheta[i-(ma[a]+MA*S),b])+deta.dtheta[i-ma[b],a]-sum(THETA%*%deta.dtheta[i-(ma[b]+MA*S),a])+ma_par%*%deta.dthetatheta[a,b,i-ma_ind]
      }
    }
    for(b in 1:Q1)
    {
      for(a in 1:q1)
      {
        deta.dthetaTHETA[a,b,i]= deta.dtheta[i-MA[b]*S,a]+RRs[a,b,(i-m)]-sum(theta%*%deta.dtheta[i-(ma+MA[b]*S),a])+deta.dTHETA[i-ma[a],b]-sum(THETA%*%deta.dTHETA[i-(ma[a]+MA*S),b])+ma_par%*%deta.dthetaTHETA[a,b,i-ma_ind]
      }
    }
    for(b in 1:P1)
    {
      for(a in 1:P1)
      {
        deta.dPHIPHI[a,b,i]<- 0 +  ma_par%*%deta.dPHIPHI[a,b,i-ma_ind]
      }
    }
    for(b in 1:q1)
    {
      for(a in 1:P1)
      {
        deta.dPHItheta[a,b,i]= deta.dPHI[i-ma[b],a] -sum(THETA%*%deta.dPHI[i-(ma[b]+MA*S),a])+ma_par%*%deta.dPHItheta[a,b,i-ma_ind]
      }
    }
    for(b in 1:Q1)
    {
      for(a in 1:P1)
      {
        deta.dPHITHETA[a,b,i]= deta.dPHI[i-MA[b]*S,a] -sum(theta%*%deta.dPHI[i-(ma+MA[b]*S),a])+ma_par%*%deta.dPHITHETA[a,b,i-ma_ind]
      }
    }
    for(b in 1:Q1)
    {
      for(a in 1:Q1)
      {
        deta.dTHETATHETA[a,b,i]= deta.dTHETA[i-MA[a]*S,b]-sum(theta%*%deta.dTHETA[i-(ma+MA[a]*S),b])+deta.dTHETA[i-MA[b]*S,a]-sum(theta%*%deta.dTHETA[i-(ma+MA[b]*S),a])+ ma_par%*%deta.dTHETATHETA[a,b,i-ma_ind]
      }
    }
  }
  ...
  KBB <- -(t(mM0)%*%mV%*%(mT^2)%*%mM0 + t(mM0)%*%mV0%*%mT2%*%mM0 + t(vI)%*%mV0%*%mT%*%mM02)
  KBq <- -(t(mM0)%*%mV%*%(mT^2)%*%qq+t(mM0)%*%mV0%*%mT2%*%qq + t(vI)%*%mV0%*%mT%*%B0q)
  KBP <- -(t(mM0)%*%mV%*%(mT^2)%*%PP+ t(mM0)%*%mV0%*%mT2%*%PP + t(vI)%*%mV0%*%mT%*%B0P)
  KBQ <- -(t(mM0)%*%mV%*%(mT^2)%*%QQ+t(mM0)%*%mV0%*%mT2%*%QQ + t(vI)%*%mV0%*%mT%*%B0Q)
  KBalpha <- -t(mM0)%*% mualpha %*% mT %*% vI#cbind(rep(0,(k+1))) 
  KqB <- t(KBq)
  ...
  KqQ=matrix(rep(NA,q1*Q1),ncol=Q1)
  if(length(KqQ)==1){
    KqQ <- -(t(qq)%*%mV%*%(mT^2)%*%QQ+t(qq)%*%mV0%*%mT2%*%QQ + t(vI)%*%mV0%*%mT%*%qQ)
  }else{
    for(j in 1:Q1){
      for(i in 1:q1){
  ...
  K <- rbind(
    cbind(KBB,KBq,KBP,KBQ,KBalpha),
    cbind(KqB,Kqq,KqP,KqQ,Kqalpha),
    cbind(KPB,KPq,KPP,KPQ,KPalpha),
    cbind(KQB,KQq,KQP,KQQ,KQalpha),
    cbind(KalphaB,Kalphaq,KalphaP,KalphaQ,Kalphaalpha)
  )
  return(K)
  }
  K<-obs.inf(y1,muhat)
  ...
  #stop("Analytic Observed Information Matrix is not positive semi-definite")
  #print("Analytic Observed Information Matrix is not positive semi-definite")
  #print(K)
  ...
  for (i in 1:length(v))
  {
    if(is.na(v[i]) | is.nan(v[i]) | v[i]<0 )  {
      #print (paste("problem: diagonal elements must be non-negative, element",i))
      # print (v[i])
      z$RMC=1
      warning("Observed Information Matrix is not positive semi-definite")
      return(z)#if Observed Information Matrix is not positive semi-definite, do not calculate
    }
  }
  
  z$zstat<-z$coeff/sqrt(v)#Estatísticas Z do Teste de Wald
  ...
  #return(z) #-#-#-# #-#-#-# #-#-#-# #-#-#-# #-#-#-# #-#-#-# #-#-#-# #-#-#-#TIRAR DEPOIS SIMULACAO #-#-#-# #-#-#-# #-#-#-# #-#-#-# #-#-#-# #-#-#-# #-#-#-# #-#-#-#
  
  
  z$pvalues<-(1-pnorm(abs(z$zstat)))*2
  
  if(is.na(exvar[1,1])==T)
  {
    first_col<-c("intercept", ma, AR, MA,"alpha estimator")
    result <- matrix(c(first_col,round(c(z$coeff,z$zstat,LI,LS,z$pvalues),4),resp), nrow=length(opt$par), ncol=7, byrow=F)
    colnames(result) <- c("Estimator","MLE","Wald's Statistic","Lower bound","Upper bound","p-value","Wald'S Test result")
    rownames(result)<-c("", rep("ma",length(ma)), rep("AR",length(AR)), rep("MA",length(MA)),"")
  }else{
    first_col<-c("intercept", 1:ncol(exvar), ma, AR, MA,"alpha estimator")
    result <- matrix(c(first_col,round(c(z$coeff,z$zstat,LI,LS,z$pvalues),4),resp), nrow=length(opt$par), ncol=7, byrow=F)
    colnames(result) <- c("Estimator","MLE","Wald's Statistic","Lower bound","Upper bound","p-value","Wald'S Test result")
    rownames(result)<-c("", rep("cov",ncol(exvar)), rep("ma",length(ma)), rep("AR",length(AR)), rep("MA",length(MA)),"")
  }
  #print(result,quot=F)
  ...
  if(m<S){MdRAE<-NA;MASE.star<-NA
  warning("m<S")}else{
    MdRAE<-median(abs(y[(m+1):n]-z$fitted[(m+1):n])/abs(y[(m+1):n]-y[(m+1-S):(n-S)]))#seasonal, if not, -1 not -S, If our model’s forecast equals to the benchmark’s forecast then the result is 1. If the benchmarks forecast are better than ours then the result will be above > 1. If ours is better than it’s below 1.
    MAEnaive.star<-sum(abs(y[(m+1):(n)]-y[(m+1-S):(n-S)]))/(n-m)
  ...
  for(i in 1:steps)
  {
    ynew_prev[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) + sum(z$Phi*(ynew_prev[n+i-AR*S]) ) - sum(ma_par*errorhat[n+i-ma_ind])
  ...
  for(i in 1:steps)
  {
    gy_prev[n+i] <- X_prev[n+i,1]*z$beta0  + sum(z$Phi*(gy[n+i-AR*S]) ) - sum(ma_par*errorhat[n+i-ma_ind])
  ...
  z$deviance <- 2*sum(dt)
  z$dof.dev=(n-m-length(opt$par)+2)#desconsidera intercepto do eta e alpha da distribuição
  z$p_deviance <- 1 - pchisq(z$deviance, z$dof.dev)
  z$vcov=sol

  z$deviance.star <- 2*sum(dt)*n/(n-m)
  z$dof.dev.star=(n-length(opt$par)+2)
  z$p_deviance.star <- 1 - pchisq(z$deviance.star, z$dof.dev.star)
  z$delta=m
  ...
  for(i in m:1)
  {
    etam[i] <- X[i,1]%*%as.matrix(z$beta0) + sum(PHI*(ynew[i+S*AR])) - sum(ma_par*errorm[i+ma_ind])
  ...
  ljungbox<- Box.test(residual, lag = aclag, type = "Ljung-Box", fitdf=(q1+P1+Q1))
  boxpierce<-Box.test(residual, lag = aclag, type = c( "Box-Pierce"), fitdf=(q1+P1+Q1))#Box-Pierce test
  monti<- Monti.test(residual, lag = aclag, type = "Ljung-Box", fitdf=(q1+P1+Q1))#Ljung-Box test as the Box.test function using pacf replacing acf
