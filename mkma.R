...
EMV.mkma <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                     tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,]%*%as.matrix(beta) - sum(theta*error[i-ma])
  ...
  score <- function(z) 
  {
    ...
    Utheta <- matrix(0, ncol=ncol(Ubeta),nrow=nrow(Ubeta))
    ...
    ##############################################################################
    score<-c(Ubeta,Utheta,Ualpha)#ma
    return(score)
  }#fim score
...
#  opt2 <- optim(reg, loglik, method = "BFGS", hessian = T, control = list(fnscale = -1))
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,]%*%as.matrix(beta) - sum(theta*errorhat[i-ma])
  ...
  B0 <- matrix(rep(NA,(n-m)*length(beta)),ncol=length(beta))#intercepto e covariáveis
  for(i in 1:(n-m))
  {
    for(j in 1:length(beta))
    {
  ...
  deta.dthetatheta<-array(0,dim=c(q1,q1,n))
    ...
    for(i in (m+1):n)
    {
      ...
      for(a in 1:q1)
      {
        deta.dbeta0theta[i,a] = deta.dbeta0[i-ma[a],] +sum(theta%*%deta.dbeta0theta[i-ma,a])
      }
      ...
      for(b in 1:q1)
      {
        for(a in 1:q1)
        {
          deta.dthetatheta[a,b,i]= deta.dtheta[i-ma[a],b]+deta.dtheta[i-ma[b],a]+sum(theta%*%deta.dthetatheta[a,b,i-ma])
        }
      }
      }
  ...
  rownames(accuracy) <- c("")
    ...
    for(i in m:1)
    {
      etam[i] <- X[i,1]%*%as.matrix(z$beta0) - sum(theta*errorm[i+ma])
  ...
  for(i in 1:steps)
  {
    ynew_prev[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) - sum(z$theta*errorhat[n+i-ma])
  ...
  for(i in 1:steps)
  {
    gy_prev[n+i] <- X_prev[n+i,1]*z$beta0  - sum(z$theta*errorhat[n+i-ma])
