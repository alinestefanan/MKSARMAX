...
EMV.mkarSAR<- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                       tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    ...
    alpha <- z[(p1+P1+k+2)] # alpha parameter
  ...
  score <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,]%*%as.matrix(beta) + sum(ar_par*(ynew[i-ar_ind] ) )
      error[i] <- ynew[i]-eta[i] #residuals on the predictor scale
    }
    ...
    for(i in 1:(n-m))
    {
      ...
      critical[i]<-(alpha-alpha/mu[i])
      if(is.na(critical[i])) critical[i]<- -.Machine$double.eps
      if(is.nan(critical[i])){critical[i]=-36.04365}##- infinito substitur por -36.04365
      if(critical[i] < -36.04365) critical[i] <- -36.04365
      den.cr[i]=log(1-exp(critical[i]))
      if(is.nan(den.cr[i])){den.cr[i]=-36.04365}#=log(1-exp(-.Machine$double.eps))=log(.Machine$double.eps), porque log(1-exp(0)) daria -infinito
    }
    ...
    B <- matrix(rep(NA,(n-m)*length(beta)),ncol=length(beta))
    for(i in 1:(n-m))
    {
    ...
    return(c(Ubeta,Uphi,UPHI,Ualpha))
  }#fim score
  ...
  x <- cbind(X[(m+1):n,],P.phi,P.PHI)#intercepto, covariáveis,ar, ma, AR, MA
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
...
#  library(rootSolve)
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,]%*%as.matrix(beta) + sum(ar_par*(ynew[i-ar_ind]) )
  ...
  for(i in (m+1):n)
  {
    deta.dbeta0[i,]<-  B0[(i-m),] 
    deta.dphi[i,]<- A[(i-m),] 
    deta.dPHI[i,]<- As[(i-m),] 
    deta.dbeta0beta0[i,]<- 0 
    deta.dbeta0phi[i,]<- rep(0,p1) 
    deta.dbeta0PHI[i,]<- rep(0,P1) 
    for(b in 1:p1)
    {
      for(a in 1:p1)
      {
        deta.dphiphi[a,b,i]<- 0 
      }
    }
    for(b in 1:P1)
    {
      for(a in 1:p1)
      {
        deta.dphiPHI[a,b,i]<- aAs[a,b,(i-m)] 
      }
    }
    for(b in 1:P1)
    {
  ...
  Kalphap <- t(Kpalpha)
  KalphaP <- t(KPalpha)
  Kalphaalpha <- -Ualphaalpha# corrigir para - ESPERANÇA DA SEGUNDA DERIVADA DA LOGVEROSSIMILHANÇA EM RELAÇÃO A alpha
  
  K <- rbind(
    cbind(KBB,KBp,KBP,KBalpha),
    cbind(KpB,Kpp,KpP,Kpalpha),
    cbind(KPB,KPp,KPP,KPalpha),
    cbind(KalphaB,Kalphap,KalphaP,Kalphaalpha)
  )
  return(K)
  }
  K<-obs.inf(y1,muhat)
  ...
  #print("Variância assintótica dos esimadores")
  #print(v)
...
#  return(z) #-#-#-# #-#-#-# #-#-#-# #-#-#-# #-#-#-# #-#-#-# #-#-#-# #-#-#-#TIRAR DEPOIS SIMULACAO #-#-#-# #-#-#-# #-#-#-# #-#-#-# #-#-#-# #-#-#-# #-#-#-# #-#-#-#
  ...
  {
    first_col<-c("intercept",ar, AR,"alpha estimator")
    ...
    rownames(result)<-c("", rep("ar",length(ar)), rep("AR",length(AR)),"")
  }else{
    first_col<-c("intercept", 1:ncol(exvar),ar, AR, "alpha estimator")
    result <- matrix(c(first_col,round(c(z$coeff,z$zstat,LI,LS,z$pvalues),4),resp), nrow=length(opt$par), ncol=7, byrow=F)
    colnames(result) <- c("Estimator","MLE","Wald's Statistic","Lower bound","Upper bound","p-value","Wald'S Test result")
    rownames(result)<-c("", rep("cov",ncol(exvar)), rep("ar",length(ar)), rep("AR",length(AR)),"")
  }
  #print(result,quot=F)
  ...
  rownames(accuracy) <- c("")
   ...
   for(i in m:1)
   {
     etam[i] <- X[i,1]%*%as.matrix(z$beta0) + sum(ar_par*(ynew[i+ar_ind]))
  ...
  for(i in 1:steps)
  {
    ynew_prev[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) + sum(ar_par*(ynew_prev[n+i-ar_ind]) ) 
  ...
  for(i in 1:steps)
  {
    gy_prev[n+i] <- X_prev[n+i,1]*z$beta0  + sum(ar_par*(gy[n+i-ar_ind]) ) 
  ...
  ljungbox<- Box.test(residual, lag = aclag, type = "Ljung-Box", fitdf=(p1+P1))
  boxpierce<-Box.test(residual, lag = aclag, type = c( "Box-Pierce"), fitdf=(p1+P1))#Box-Pierce test
  monti<- Monti.test(residual, lag = aclag, type = "Ljung-Box", fitdf=(p1+P1))#Ljung-Box test as the Box.test function using pacf replacing acf
