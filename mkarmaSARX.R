...
EMV.mkarmaSARX <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                           tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    ...
    alpha <- z[(p1+P1+q1+k+2)] # alpha parameter
  ...
  score <- function(z) 
  {
    ...
    for(i in 1:(n-m))
    {
      for(j in 1:p1)
      {
        ...
        )-sum(PHI%*%(ynew[i+m-(S*AR+ar[j])]-X[i+m-(S*AR+ar[j]),2:ncol(X)]%*%as.matrix(beta)
        ))
      }
    }    
    ...
    for(i in 1:(n-m))
    {
      for(j in 1:P1)
      {
        ...
        )-sum(phi%*%(ynew[i+m-(S*AR[j]+ar)]-X[i+m-(S*AR[j]+ar),2:ncol(X)]%*%as.matrix(beta)
        ))
      }
    }    
    ...
    return(c(Ubeta0,Ubeta,Uphi,Utheta,UPHI,Ualpha))
  }#end score
  ...
  x <- cbind(X[(m+1):n,],P.phi, Q.theta ,P.PHI)#intercepto, covariáveis,ar, ma, AR
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
  ...
  alpha <-coef[(p1+P1+q1+k+2)] # alpha parameter
  ...
  z$MA=MA
  z$delta=m
  z$roots=c(abs(polyroot(vector.root(z$ar,z$phi))),abs(polyroot(vector.root(z$ma,z$theta))),abs(polyroot(vector.root(z$AR,z$Phi))))
  z$RMC=0
  ...
  obs.inf<-function(y1,muhat)
  {
    ...
    for(i in (m+1):n)
    {
      ...
      for(a in 1:q1)
      {
        deta.dbeta0theta[i,a] = deta.dbeta0[i-ma[a],] +ma_par%*%deta.dbeta0theta[i-ma_ind,a]
      }
      deta.dbeta0PHI[i,]<- rep(0,P1) +  ma_par%*%deta.dbeta0PHI[i-ma_ind,]
      for(b in 1:k)
      {
      ...
      for(b in 1:q1)
      {
        for(a in 1:k)
        {
          deta.dbetatheta[a,b,i] <- deta.dbeta[i-ma[b],a] + ma_par%*%deta.dbetatheta[a,b,i-ma_ind]
        }
      }
      for(b in 1:P1)
      {
      ...
      for(b in 1:q1)
      {
        for(a in 1:p1)
        {
          deta.dphitheta[a,b,i]= deta.dphi[i-ma[b],a]+ma_par%*%deta.dphitheta[a,b,i-ma_ind]
        }
      }
      for(b in 1:P1)
      {
        for(a in 1:p1)
        {
          deta.dphiPHI[a,b,i]<- aAs[a,b,(i-m)] +  ma_par%*%deta.dphiPHI[a,b,i-ma_ind]
        }
      }
      for(b in 1:q1)
      {
        for(a in 1:q1)
        {
          deta.dthetatheta[a,b,i]= deta.dtheta[i-ma[a],b]+deta.dtheta[i-ma[b],a]+ma_par%*%deta.dthetatheta[a,b,i-ma_ind]
        }
      }
      for(b in 1:P1)
      {
    ...
    ########################################################################################################### 
    ####END DERIVATIVE FROM LOG LIKELIHOOD WITH RESPECT TO MU 
    # print("mu.eta(etahat[(m+1):n])");print(mu.eta(etahat[(m+1):n]))
    ...
    K <- rbind(
      cbind(KB0B0,KB0B,KB0p,KB0q,KB0P,KB0alpha),
      cbind(KBB0,KBB,KBp,KBq,KBP,KBalpha),
      cbind(KpB0,KpB,Kpp,Kpq,KpP,Kpalpha),
      cbind(KqB0,KqB,Kqp,Kqq,KqP,Kqalpha),
      cbind(KPB0,KPB,KPp,KPq,KPP,KPalpha),
      cbind(KalphaB0,KalphaB,Kalphap,Kalphaq,KalphaP,Kalphaalpha)
    )
    return(K)
  }
  K<-obs.inf(y1,muhat)
  ...
  first_col<-c("intercept", 1:k,ar, ma, AR,"alpha estimator")
  ...
  rownames(result)<-c("", rep("cov",k), rep("ar",length(ar)), rep("ma",length(ma)), rep("AR",length(AR)),"")
  ...
  z$deviance <- 2*sum(dt)
  z$dof.dev=(n-m-p1-q1-P1)#desconsidera intercepto do eta e alpha da distribuição
  ...
  ljungbox<- Box.test(residual, lag = aclag, type = "Ljung-Box", fitdf=(p1+q1+P1))
  boxpierce<-Box.test(residual, lag = aclag, type = c( "Box-Pierce"), fitdf=(p1+q1+P1))#Box-Pierce test
  monti<- Monti.test(residual, lag = aclag, type = "Ljung-Box", fitdf=(p1+q1+P1))#Ljung-Box test as the Box.test function using pacf replacing acf
