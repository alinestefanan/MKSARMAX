...
EMV.mkmaX <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                        tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,1]*beta0 + X[i,2:ncol(X)]%*%as.matrix(beta) - sum(theta*error[i-ma])
  ...
  x <- cbind(X[(m+1):n,], Q.theta)#intercepto, covariáveis,ar, ma
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
 ...
 # library(rootSolve)
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,1]*beta0 + X[i,2:ncol(X)]%*%as.matrix(beta) - sum(theta*errorhat[i-ma])
  ...
  obs.inf<-function(y1,muhat)
  {
    ...
    for(i in (m+1):n)
    {
      ...
      for(b in 1:k)
      {
        for(a in 1:k)
        {
          deta.dbetabeta[a,b,i] <- 0 + sum(theta%*%deta.dbetabeta[a,b,i-ma])
        }
      }
      for(b in 1:q1)
      {
        for(a in 1:k)
        {
          deta.dbetatheta[a,b,i] <- deta.dbeta[i-ma[b],a] + sum(theta%*%deta.dbetatheta[a,b,i-ma])
        }
      }
      for(b in 1:q1)
      {
  ...
  for(i in m:1)
  {
    etam[i] <- X[i,1]*z$beta0 +X[i,2:ncol(X)]%*%as.matrix(z$beta)- sum(theta*errorm[i+ma])
  ...
  for(i in 1:steps)
  {
    ynew_prev[n+i] <- X_prev[n+i,1]*z$beta0 + X_prev[n+i,2:ncol(X_prev)]%*%as.matrix(z$beta) - sum(theta*errorhat[n+i-ma])
