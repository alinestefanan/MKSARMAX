...
EMV.mkarmaSARMAX <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                             tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    ...
    PHI = z[(p1+q1+k+2):(p1+q1+P1+k+1)]
    THETA = z[(p1+P1+q1+k+2):(p1+P1+q1+Q1+k+1)]
    alpha <- z[(p1+P1+q1+Q1+k+2)] # alpha parameter
  ...
  score <- function(z) 
  {
    ...
    for(i in 1:(n-m))
    {
      #critical[i] <- exp(alpha-alpha/mu[i])      # Replicate example vector
      if(is.na(mu[i]))mu[i]<-.Machine$double.eps
      if(mu[i]<.Machine$double.eps) mu[i]<-.Machine$double.eps
      if(mu[i]>(0.9999999)) mu[i]<-0.9999999#1-.Machine$double.eps
      #if(is.na(critical[i]))critical[i]<-.Machine$double.eps
      #if(critical[i]<.Machine$double.eps) critical[i]<-.Machine$double.eps 
      #if(critical[i]>(1-.Machine$double.eps)) critical[i]<-1-.Machine$double.eps
      critical[i]<-(alpha-alpha/mu[i])
      if(is.na(critical[i])) critical[i]<- -.Machine$double.eps
      if(is.nan(critical[i])){critical[i]=-36.04365}
      if(critical[i] < -36.04365) critical[i] <- -36.04365
    ...
    for(i in 1:(n-m))
    {
      for(j in 1:p1)
      {
        A[i,j] <- (ynew[i+m-ar[j]]-X[i+m-ar[j],2:ncol(X)]%*%as.matrix(beta))-sum(PHI%*%(ynew[i+m-(S*AR+ar[j])]-X[i+m-(S*AR+ar[j]),2:ncol(X)]%*%as.matrix(beta)
        ))
      }
    }    
    ...
    for(i in 1:(n-m))
    {
      for(j in 1:P1)
      {
        As[i,j] <- (ynew[i+m-(S*AR[j])]-X[i+m-(S*AR[j]),2:ncol(X)]%*%as.matrix(beta))-sum(phi%*%(ynew[i+m-(S*AR[j]+ar)]-X[i+m-(S*AR[j]+ar),2:ncol(X)]%*%as.matrix(beta)
        ))
      }
    }    
    ...
    ##############################################################################
    return(c(Ubeta0,Ubeta,Uphi,Utheta,UPHI,UTHETA,Ualpha))
  }#end score
  ...
  x <- cbind(X[(m+1):n,],P.phi, Q.theta ,P.PHI,Q.THETA)#intercepto, covariáveis,ar, ma, AR, MA
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
  ...
  z$conv <- opt$conv
  coef <- (opt$par)[1:(p1+P1+q1+Q1+k+2)]
  z$coeff <- coef
  z$loglik <- opt$value
  ...
  PHI <-coef[(p1+q1+k+2):(p1+q1+P1+k+1)] #AR
  THETA <-coef[(p1+P1+q1+k+2):(p1+P1+q1+Q1+k+1)] #MA
  alpha <-coef[(p1+P1+q1+Q1+k+2)] # alpha parameter
  ...
  obs.inf<-function(y1,muhat)
  {
    ...
    aAs <- array(NA,c(p1,P1,(n-m)))# arSAR
    for(i in 1:(n-m))
    {
      for(b in 1:P1)
      {
    ...
    for(i in (m+1):n)
    {
      ...
      for(b in 1:P1)
      {
        for(a in 1:p1)
        {
          deta.dphiPHI[a,b,i]<- aAs[a,b,(i-m)] +  sum(ma_par%*%deta.dphiPHI[a,b,i-ma_ind])
        }
      }
      for(b in 1:Q1)
      {
      ...
      for(b in 1:Q1)
      {
        for(a in 1:q1)
        {
          deta.dthetaTHETA[a,b,i]= deta.dtheta[i-MA[b]*S,a]+RRs[a,b,(i-m)]-sum(theta%*%deta.dtheta[i-(ma+MA[b]*S),a])+deta.dTHETA[i-ma[a],b]-sum(THETA%*%deta.dTHETA[i-(ma[a]+MA*S),b])+sum(ma_par%*%deta.dthetaTHETA[a,b,i-ma_ind])
        }
      }
      for(b in 1:P1)
      {
    ...
    K <- rbind(
      cbind(KB0B0,KB0B,KB0p,KB0q,KB0P,KB0Q,KB0alpha),
      cbind(KBB0,KBB,KBp,KBq,KBP,KBQ,KBalpha),
      cbind(KpB0,KpB,Kpp,Kpq,KpP,KpQ,Kpalpha),
      cbind(KqB0,KqB,Kqp,Kqq,KqP,KqQ,Kqalpha),
      cbind(KPB0,KPB,KPp,KPq,KPP,KPQ,KPalpha),
      cbind(KQB0,KQB,KQp,KQq,KQP,KQQ,KQalpha),
      cbind(KalphaB0,KalphaB,Kalphap,Kalphaq,KalphaP,KalphaQ,Kalphaalpha)
    )
    return(K)
  }
  K<-obs.inf(y1,muhat)
  ...
  first_col<-c("intercept", 1:k,ar, ma, AR, MA,"alpha estimator")
  ...
  rownames(result)<-c("", rep("cov",k), rep("ar",length(ar)), rep("ma",length(ma)), rep("AR",length(AR)), rep("MA",length(MA)),"")
  ...
  z$deviance <- 2*sum(dt)
  z$dof.dev=(n-m-p1-q1-P1-Q1)#desconsidera intercepto do eta e alpha da distribuição
  ...
  if(graph==T)
  {
    ...
    dev.off()
    pdf("obs_v_fit.pdf",width=5, height=4)
    {
      par(mfrow=c(1,1))
      par(mar=c(2.8, 2.7, 1, 1)) 
      par(mgp=c(1.7, 0.45, 0))
