...
EMV.mkSAR<-  function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                      tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    beta <- z[1:(k+1)]
    PHI = z[(k+2):(P1+k+1)]
    alpha <- z[(P1+k+2)] # alpha parameter
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,]%*%as.matrix(beta) + sum(PHI*(ynew[i-AR*S]    ) )
  ...
  x <- cbind(X[(m+1):n,],P.PHI)#intercepto, covariáveis,AR
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
  ajuste = lm.fit(x, Ynew) 
  
  mqo = c(ajuste$coef)
  for(i in 1:length(mqo))#se virar NA, para então deixar 0
  {
    if (is.na(mqo[i])){
      mqo[i] <- 0
    }
...
#  print(rbind(score(opt2$par),gradient(loglik,opt2$par)))
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,]%*%as.matrix(beta) + sum(PHI*(ynew[i-AR*S]    ) )
  ...
  K <- rbind(
    cbind(KBB,KBP,KBalpha),
    cbind(KPB,KPP,KPalpha),
    cbind(KalphaB,KalphaP,Kalphaalpha)
  )
  return(K)
  }
  K<-obs.inf(y1,muhat)
  ...
  {
    first_col<-c("intercept", AR,"alpha estimator")
    ...
    rownames(result)<-c("", rep("AR",length(AR)),"")
  }else{
    first_col<-c("intercept", 1:ncol(exvar), AR, "alpha estimator")
    result <- matrix(c(first_col,round(c(z$coeff,z$zstat,LI,LS,z$pvalues),4),resp), nrow=length(opt$par), ncol=7, byrow=F)
    colnames(result) <- c("Estimator","MLE","Wald's Statistic","Lower bound","Upper bound","p-value","Wald'S Test result")
    rownames(result)<-c("", rep("cov",ncol(exvar)), rep("AR",length(AR)),"")
  }
  ...
  ### STOP error metrics
   ...
   for(i in m:1)
   {
     etam[i] <- X[i,1]%*%as.matrix(z$beta0) + sum(PHI*(ynew[i+AR*S])) 
  ...
  for(i in 1:steps)
  {
    ynew_prev[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) + sum(z$Phi*(ynew_prev[n+i-AR*S]  ) )
  ...
  for(i in 1:steps)
  {
    gy_prev[n+i] <- X_prev[n+i,1]*z$beta0  + sum(z$Phi*(gy[n+i-AR*S]) )
  ...
  ljungbox<- Box.test(residual, lag = aclag, type = "Ljung-Box", fitdf=(P1))
  boxpierce<-Box.test(residual, lag = aclag, type = c( "Box-Pierce"), fitdf=(P1))#Box-Pierce test
  monti<- Monti.test(residual, lag = aclag, type = "Ljung-Box", fitdf=(P1))#Ljung-Box test as the Box.test function using pacf replacing acf
