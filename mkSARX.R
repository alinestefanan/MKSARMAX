...
EMV.mkSARX <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                       tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  score <- function(z) 
  {
    ...
    ##############################################################################
    return(c(Ubeta0,Ubeta,UPHI,Ualpha))
  }#end score
  ...
  x <- cbind(X[(m+1):n,], P.PHI)#intercepto, covariáveis,ar, ma, AR
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
  ...
  obs.inf<-function(y1,muhat)
  {
    ...
    K <- rbind(
      cbind(KB0B0,KB0B,KB0P,KB0alpha),
      cbind(KBB0,KBB,KBP,KBalpha),
      cbind(KPB0,KPB,KPP,KPalpha),
      cbind(KalphaB0,KalphaB,KalphaP,Kalphaalpha)
    )
    return(K)
  }
  K<-obs.inf(y1,muhat)
  ...
  first_col<-c("intercept", 1:k, AR,"alpha estimator")
  ...
  rownames(result)<-c("", rep("cov",k), rep("AR",length(AR)),"")
  ...
  # for(i in m:1)
  # {
  #   etam[i] <- X[i,1]*z$beta0 +X[i,2:ncol(X)]%*%as.matrix(z$beta)+ sum(PHI*(ynew[i+S*AR])-X[i+S*AR,2:ncol(X)]%*%as.matrix(z$beta))
  ...
  z$deviance <- 2*sum(dt)
  z$dof.dev=(n-m-P1)#desconsidera intercepto do eta e alpha da distribuição
