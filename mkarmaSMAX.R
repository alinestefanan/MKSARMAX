...
EMV.mkarmaSMAX <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                           tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  score <- function(z) 
  {
    ...
    ##############################################################################
    #print("score")
    #print(c(Ubeta,Uphi,Utheta,UPHI,UTHETA,Ualpha))
    
    return(c(Ubeta0,Ubeta,Uphi,Utheta,UTHETA,Ualpha))
  }#end score
  ...
  obs.Fisher.inf<-function(y1,muhat)
  {
    ...
    for(i in (m+1):n)
    {
      ...
      for(b in 1:p1)
      {
        for(a in 1:k)
        {
          deta.dbetaphi[a,b,i] <- bA[a,b,(i-m)] + ma_par%*%deta.dbetaphi[a,b,i-ma_ind]
        }
      }
      for(b in 1:q1)
      {
    ...
    K <- rbind(
      cbind(KB0B0,KB0B,KB0p,KB0q,KB0Q,KB0alpha),
      cbind(KBB0,KBB,KBp,KBq,KBQ,KBalpha),
      cbind(KpB0,KpB,Kpp,Kpq,KpQ,Kpalpha),
      cbind(KqB0,KqB,Kqp,Kqq,KqQ,Kqalpha),
      cbind(KQB0,KQB,KQp,KQq,KQQ,KQalpha),
      cbind(KalphaB0,KalphaB,Kalphap,Kalphaq,KalphaQ,Kalphaalpha)
    )
    return(K)
  }
  K<-obs.Fisher.inf(y1,muhat)
  ...
  first_col<-c("intercept", 1:k,ar, ma, MA,"alpha estimator")
  ...
  rownames(result)<-c("", rep("cov",k), rep("ar",length(ar)), rep("ma",length(ma)), rep("MA",length(MA)),"")
  ...
  z$deviance <- 2*sum(dt)
  z$dof.dev=(n-m-p1-q1-Q1)#desconsidera intercepto do eta e alpha da distribuição
