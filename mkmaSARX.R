...
EMV.mkmaSARX <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                         tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  score <- function(z) 
  {
    ...
    ##############################################################################
    return(c(Ubeta0,Ubeta,Utheta,UPHI,Ualpha))
  }#end score
  ...
  x <- cbind(X[(m+1):n,], Q.theta ,P.PHI)#intercepto, covariáveis,ar, ma, AR
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
  ...
  z$MA=MA
  z$delta=m
  z$roots=c(abs(polyroot(vector.root(z$ma,z$theta))),abs(polyroot(vector.root(z$AR,z$Phi))))
  z$RMC=0
  ...
  obs.inf<-function(y1,muhat)
  {
    ...
    KBalpha <- -t(mM)%*% mualpha %*% mT %*% vI#cbind(rep(0,(k+1))) 
    ...
    Kqalpha <- -t(qq)%*% mualpha %*% mT %*% vI#cbind(rep(0,q1)) 
    ...
    K <- rbind(
      cbind(KB0B0,KB0B,KB0q,KB0P,KB0alpha),
      cbind(KBB0,KBB,KBq,KBP,KBalpha),
      cbind(KqB0,KqB,Kqq,KqP,Kqalpha),
      cbind(KPB0,KPB,KPq,KPP,KPalpha),
      cbind(KalphaB0,KalphaB,Kalphaq,KalphaP,Kalphaalpha)
    )
    return(K)
  }
  K<-obs.inf(y1,muhat)
  ...
  first_col<-c("intercept", 1:k, ma, AR,"alpha estimator")
  ...
  rownames(result)<-c("", rep("cov",k),  rep("ma",length(ma)), rep("AR",length(AR)),"")
  ...
  for(i in m:1)
  {
    etam[i] <- X[i,1]*z$beta0 +X[i,2:ncol(X)]%*%as.matrix(z$beta)+ sum(PHI*(ynew[i+S*AR])-X[i+S*AR,2:ncol(X)]%*%as.matrix(z$beta)) - sum(theta*errorm[i+ma])
  ...
  z$deviance <- 2*sum(dt)
  z$dof.dev=(n-m-q1-P1)#desconsidera intercepto do eta e alpha da distribuição
  ...
  ljungbox<- Box.test(residual, lag = aclag, type = "Ljung-Box", fitdf=(q1+P1))
  boxpierce<-Box.test(residual, lag = aclag, type = c( "Box-Pierce"), fitdf=(q1+P1))#Box-Pierce test
  monti<- Monti.test(residual, lag = aclag, type = "Ljung-Box", fitdf=(q1+P1))#Ljung-Box test as the Box.test function using pacf replacing acf
