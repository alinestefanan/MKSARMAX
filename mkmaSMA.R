...
EMV.mkmaSMA <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                        tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    beta <- z[1:(k+1)]
    theta =z[(k+2):(q1+k+1)]
    THETA = z[(q1+k+2):(q1+Q1+k+1)]
    alpha <- z[(q1+Q1+k+2)] # alpha parameter
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,]%*%as.matrix(beta) - sum(ma_par*error[i-ma_ind])
  ...
  score <- function(z) 
  {
    ...
    return(c(Ubeta,Utheta,UTHETA,Ualpha))
  }#fim score
  ...
  x <- cbind(X[(m+1):n,], Q.theta ,Q.THETA)#intercepto, covariáveis, ma, MA
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
 ...
 # print(rbind(score(opt2$par),gradient(loglik,opt2$par)))
  ...
  z$conv <- opt$conv
  coef <- (opt$par)[1:(q1+Q1+k+2)]
  z$coeff <- coef
  
  beta <-coef[1:(k+1)] #intercept and covariates
  theta <-coef[(k+2):(q1+k+1)] #ma
  THETA <-coef[(q1+k+2):(q1+Q1+k+1)] #MA
  alpha <-coef[(q1+Q1+k+2)] # alpha parameter
  ...
  z$MA=MA
  z$delta=m
  z$roots=c(abs(polyroot(vector.root(z$ma,z$theta))),abs(polyroot(vector.root(z$MA,z$Theta))))
  z$RMC=0
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,]%*%as.matrix(beta) - sum(ma_par*errorhat[i-ma_ind])
  ...
  for(i in (m+1):n)
  {
    deta.dbeta0[i,]<-  B[(i-m),] +  ma_par%*%deta.dbeta0[i-ma_ind,]
    deta.dtheta[i,]<- R[(i-m),] +  ma_par%*%deta.dtheta[i-ma_ind,]
    deta.dTHETA[i,]<- Rs[(i-m),] +  ma_par%*%deta.dTHETA[i-ma_ind,]
  ...
  K <- rbind(
    cbind(KBB,KBq,KBQ,KBalpha),
    cbind(KqB,Kqq,KqQ,Kqalpha),
    cbind(KQB,KQq,KQQ,KQalpha),
    cbind(KalphaB,Kalphaq,KalphaQ,Kalphaalpha)
  )
  return(K)
  }
  K<-obs.inf(y1,muhat)
 ...
 # return(z) # # # # # # # # # # # # # # tirar depois, abaixo nao precisa na simulacao # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
  ...
  {
    first_col<-c("intercept", ma, MA,"alpha estimator")
    ...
    rownames(result)<-c("", rep("ma",length(ma)), rep("MA",length(MA)),"")
  }else{
    first_col<-c("intercept", 1:ncol(exvar), ma, MA,"alpha estimator")
    result <- matrix(c(first_col,round(c(z$coeff,z$zstat,LI,LS,z$pvalues),4),resp), nrow=length(opt$par), ncol=7, byrow=F)
    colnames(result) <- c("Estimator","MLE","Wald's Statistic","Lower bound","Upper bound","p-value","Wald'S Test result")
    rownames(result)<-c("", rep("cov",ncol(exvar)), rep("ma",length(ma)), rep("MA",length(MA)),"")
  }
  #print(result,quot=F)
  ...
  for(i in 1:steps)
  {
    ynew_prev[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) - sum(ma_par*errorhat[n+i-ma_ind])
  ...
  for(i in 1:steps)
  {
    gy_prev[n+i] <- X_prev[n+i,1]*z$beta0 - sum(ma_par*errorhat[n+i-ma_ind])
  ...
  for(i in m:1)
  {
    etam[i] <- X[i,1]%*%as.matrix(z$beta0) - sum(ma_par*errorm[i+ma_ind])
  ...
  ljungbox<- Box.test(residual, lag = aclag, type = "Ljung-Box", fitdf=(q1+Q1))
  boxpierce<-Box.test(residual, lag = aclag, type = c( "Box-Pierce"), fitdf=(q1+Q1))#Box-Pierce test
  monti<- Monti.test(residual, lag = aclag, type = "Ljung-Box", fitdf=(q1+Q1))#Ljung-Box test as the Box.test function using pacf replacing acf
  ...
  library(seastests)
  friedman=fried(ts(y[1:n],frequency=S))#Friedman Rank test
