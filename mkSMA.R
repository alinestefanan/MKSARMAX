...
EMV.mkSMA <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
                      tau=0.5,resid=1,aclag=10,steps=12,validation=T,graph=T,print=T,check=F,link="logit")
{
  ...
  loglik <- function(z) 
  {
    beta <- z[1:(k+1)]
    THETA = z[(k+2):(Q1+k+1)]
    alpha <- z[(Q1+k+2)] # alpha parameter
    ...
    for(i in (m+1):n)
    {
      eta[i] <- X[i,]%*%as.matrix(beta) - sum(THETA*error[i-MA*S])
  ...
  x <- cbind(X[(m+1):n,],Q.THETA)#intercepto, covariáveis, MA
  y1<-y[(m+1):n] 
  Ynew = linkfun(y1) 
 ...
 # print(rbind(score(opt2$par),gradient(loglik,opt2$par)))
  ...
  for(i in (m+1):n)
  {
    etahat[i] <- X[i,]%*%as.matrix(beta) - sum(THETA*errorhat[i-MA*S])
  ...
  deta.dTHETATHETA<-array(0,dim=c(Q1,Q1,n))
   ...
   for(i in (m+1):n)
   {
     ...
     for(b in 1:Q1)
     {
       deta.dbeta0THETA[i,b] = deta.dbeta0[i-MA[b]*S,] + sum(THETA%*%deta.dbeta0THETA[i-MA*S,b])
     }
     for(b in 1:Q1)
     {
       for(a in 1:Q1)
       {
         deta.dTHETATHETA[a,b,i]= deta.dTHETA[i-MA[a]*S,b]+deta.dTHETA[i-MA[b]*S,a]+ sum(THETA%*%deta.dTHETATHETA[a,b,i-MA*S])
       }
     } 
   }
  ...
  K <- rbind(
    cbind(KBB,KBQ,KBalpha),
    cbind(KQB,KQQ,KQalpha),
    cbind(KalphaB,KalphaQ,Kalphaalpha)
  )
  return(K)
  }
  K<-obs.inf(y1,muhat)
  ...
  {
    first_col<-c("intercept", MA,"alpha estimator")
    ...
    first_col<-c("intercept", 1:ncol(exvar), MA,"alpha estimator")
    result <- matrix(c(first_col,round(c(z$coeff,z$zstat,LI,LS,z$pvalues),4),resp), nrow=length(opt$par), ncol=7, byrow=F)
    colnames(result) <- c("Estimator","MLE","Wald's Statistic","Lower bound","Upper bound","p-value","Wald'S Test result")
    rownames(result)<-c("", rep("cov",ncol(exvar)), rep("MA",length(MA)),"")
  }
  ...
  for(i in m:1)
  {
    etam[i] <- X[i,1]%*%as.matrix(z$beta0) - sum(THETA*errorm[i+S*MA])
  ...
  for(i in 1:steps)
  {
    ynew_prev[n+i] <- X_prev[n+i,]%*%as.matrix(z$beta0) - sum(z$Theta*errorhat[n+i-MA*S])
  ...
  for(i in 1:steps)
  {
    gy_prev[n+i] <- X_prev[n+i,1]*z$beta0 - sum(z$Theta*errorhat[n+i-MA*S])
  ...
  monti<- Monti.test(residual, lag = aclag, type = "Ljung-Box", fitdf=(Q1))#Ljung-Box test as the Box.test function using pacf replacing acf
