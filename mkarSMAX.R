...
EMV.mkarSMAX <- function(y,ar=c(0.0),ma=c(0.0),AR=c(0.0),MA=c(0.0),S=12,exvar=matrix(NA, nrow=1, ncol=1, byrow=F),
...
#  print(rbind(score(opt2$par),gradient(loglik,opt2$par)))
 ...
 critical<-c()
  ...
  for(i in m:1)
  {
    etam[i] <- X[i,1]*z$beta0 +X[i,2:ncol(X)]%*%as.matrix(z$beta)+ sum(phi*(ynew[i+ar])-X[i+ar,2:ncol(X)]%*%as.matrix(z$beta)) - sum(THETA*errorm[i+S*MA])
